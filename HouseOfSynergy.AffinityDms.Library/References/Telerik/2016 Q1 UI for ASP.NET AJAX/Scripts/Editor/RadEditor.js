...
},getValue:function(f){f=f||this._window;
try{var d=this._getBlockElement(f);
var g=this._getTextValue(d);
return g||"Normal";
}catch(e){}return null;
},_getBlockElement:function(f){var g=b.Editor.DomRange.rangeFromWindow(f);
var d=g&&g.getParentElement();
if(d){if(typeof(d.tagName)=="undefined"){d=d.parentNode;
}var e=c.getElementParentByRegExpTagName(d,/PRE|MENU|DIR|ADDRESS/i);
if(e){d=e;
}else{if(!c.isBlockElement(d)){d=c.getBlockParent(d);
}}}return d;
},_getTextValue:function(f){var e=this._defaultBlocks;
var j=f&&f.tagName||"";
if(j){for(var g=0;
g<e.length;
g++){var d=e[g];
...
}if(e.ff){k.fixDeleteEmptyBlock(i,j);
...
if(C.get_enableTrackChanges()&&C.get_enableTrackChangesOverride()){C._clearTrackedInlineAncestors(E,false);
...
}},_replaceSpanWithElement:function(q,y,C){var B=this._getElements(q,"span");
var r=[];
for(var t=B.length-1;
t>=0;
t--){Array.add(r,B[t]);
...
if(z==C||z==(C+";")){var x=q.ownerDocument.createElement(y);
for(var v=0;
v<p.length;
v++){x.appendChild(p[v]);
...
b.MozillaRemoveLastBrFilter.prototype={getHtmlContent:function(p){this._removeLastBr(p);
return p;
},_removeLastBr:function(q){if(!q||$telerik.isIE){return;
}var p=(q.tagName=="HTML")?q.getElementsByTagName("BODY")[0]:q;
this._removeLasBrInNode(p);
if($telerik.isFirefox){this._removeLasBrInTD(p);
}},_removeLasBrInTD:function(p){var q=Function.createDelegate(this,this._removeLasBrInNode);
$telerik.$("td",p).each(function(r,s){q(s);
});
},_removeLasBrInNode:function(q){var p=(q.lastChild)?q.lastChild:null;
if(p&&p.tagName&&"BR"==p.tagName){b.Utils.removeNode(p);
...
if($telerik.isFirefox){this._fixFontFamilyValues(x,t);
...
if($telerik.isFirefox){var u=this._getFontFamilyValue(w.attributes.style.nodeValue);
s.face=u?u:s.face;
...
if(!q){return q;
}if($telerik.isFirefox){this.reverseNodeAttributes(p);
}return p.attributes;
...
if("href"==p&&q&&!/^https?\:\/\//i.test(q)){x.innerHTML='<a href="'+q.replace(/\"/gi,"%22").replace(/[\s\u00a0]/gi,"%20")+'">test</a>';
var v;
...
}}}else{if("src"==p&&q){x.innerHTML='<img src="'+q.replace(/\"/gi,"%22").replace(/[\s\u00a0]/gi,"%20")+'" />';
...
g(k);
},getRange:function(k){var m=this;
var l;
try{l=b.DomRange.rangeFromWindow(m.Window);
}catch(j){}return k?l:l||b.DomRange.createRange(m.Document);
...
(function(a,H){var i=window.document;
var A=a.proxy;
var c=Telerik.Web.UI;
var b=c.Editor;
var m=b.EditorEditType;
var l=c.EditorContentAreaMode;
var I=b.Utils;
var t=I.isTag;
var n=c.EventType;
var o="focus";
...
var y="Paste";
var E="resizeEnd";
var r="reInlineEditor";
var s="reInlineEditorSelected";
var p="reFullScreen";
var q="inlineEditCompleted";
var j=".telerikEditor";
var k=".Editor-img";
var f=".reContent";
var w="(orientation: portrait)";
var x="rePortrait";
var v="reLandScape";
var G=".reToolbar,.reToolBar";
var u=b.Orientation;
var F="matchMedia" in window;
var z=Telerik.Web.Platform;
var e=Telerik.Web.Browser;
var D=c.RenderMode;
var d=Sys.UI.DomElement.addCssClass;
var C=Sys.UI.DomElement.removeCssClass;
var h=Function.createDelegate;
var B=/[\u200b]/g;
c.RadEditor=function(K){var J=this;
c.RadEditor.initializeBase(this,[K]);
...
J._onHiddenTextAreaFocusDelegate=null;
...
J._animationSettings={};
};
c.RadEditor.createNew=function(K){var J=new c.RadEditor(i.createElement("div"));
if(!K){J.get_contentAreaElement();
}else{J._contentAreaElement=K;
}J._contentHiddenTextarea=i.createElement("input");
J._initContentAreaHtml("",true);
J.set_editable(true);
J._ieHookToClientArea();
J._addDefaultShortCuts();
J.removeShortCut("ToggleScreenMode");
J.removeShortCut("LinkManager");
J.removeShortCut("ImageManager");
J.removeShortCut("SetToolFocus");
return J;
};
c.RadEditor.prototype={_initializeModulesManager:function(){var J=this;
if(typeof(b.ModulesManager)==="function"&&!J._isInInlineEditMode()){J._modulesManager=new b.ModulesManager(J);
}},_isInInlineEditMode:function(){var J=this;
return(J.get_editType()===m.Inline);
},get_ajaxSpellCheck:function(){return this._ajaxSpellCheck;
},set_ajaxSpellCheck:function(J){this._ajaxSpellCheck=J;
...
},addCustomWord:function(J){this.get_spellCheckService().addCustomWord(J);
},_responseReceived:function(K,J){},raiseEvent:function(K,J){if(K!="selectionChange"||this.get_mode()==c.EditModes.Design){c.RadEditor.callBaseMethod(this,"raiseEvent",[K,J]);
}},_initializeToolProvider:function(){var K=this;
var W=K.get_toolProviderID();
var O=W?$find(W):null;
if(!O){return;
}var L=function(Y){var Z=Y.get_window();
return Z?Z.get_contentElement():Y.get_element();
};
var U=K.get_toolAdapter();
var P=O?O.get_toolAdapter():null;
var R=L(P);
var Q=R?a(R).find(G):[];
var S=Q.length;
var X=i.createDocumentFragment();
K.set_dialogOpener(O.get_dialogOpener());
var J=O.get_contextMenusJSON();
if(J&&J.concat){K.set_contextMenusJSON(J.concat([]));
}var N=O.get_modulesJSON();
if(N&&N.concat){K.set_modulesJSON(N.concat([]));
}K.set_toolJSON(O.get_toolJSON());
U.set_toolJSON(O.get_toolAdapter().get_toolJSON());
var V=L(U);
for(var M=0;
M<S;
M++){a(Q[M]).clone().appendTo(X);
}a(V).append(X);
if(O.get_spellCheckJSON){K.set_spellCheckJSON(O.get_spellCheckJSON());
}if(O.get_ajaxSpellCheckScriptReference){var T=O.get_ajaxSpellCheckScriptReference();
if(T){K.set_ajaxSpellCheckScriptReference(T);
}}K.set_links(O.get_links());
K.set_colors(O.get_colors());
K.set_fontNames(O.get_fontNames());
K.set_fontSizes(O.get_fontSizes());
K.set_symbols(O.get_symbols());
K.set_realFontSizes(O.get_realFontSizes());
K.set_snippets(O.get_snippets());
K.set_cssClasses(O.get_cssClasses());
K.set_cssFiles(O.get_cssFiles());
K.set_languages(O.get_languages());
K.set_paragraphs(O.get_paragraphs());
K.set_formatSets(O.get_formatSets());
K.set_enableUndoRedo(O.get_enableUndoRedo());
},get_toolHTML:function(){var J=this.get_toolContainer().cloneNode(true);
return J.innerHTML;
...
},_ieHookToClientArea:function(){var J=this;
window.setTimeout(function(){if($telerik.isIE){var L=J.get_contentArea();
if(!L){return;
}try{var O=c.Editor.Utils.storeBrowserPosition();
var K=i.activeElement;
if(K&&K.nodeName===J.get_contentAreaElement()){var N=i.body.createTextRange();
if(O&&O.y>0){N.moveToElementText(J.get_contentAreaElement());
}else{N.moveStart("textedit",N.text.length);
}N.collapse(true);
N.select();
if(O&&O.y>0){c.Editor.Utils.restoreBrowserPosition();
}}}catch(M){}}},0);
},_isEditorRightToLeft:function(){var J=this._isRightToLeft;
if(J==null){var K=this.get_element();
if(K){J=this._isRightToLeft=$telerik.isRightToLeft(K);
}}return J;
},initialize:function(N){var J=this;
var O;
J._cleanZeroWidthSpace=true;
c.RadEditor.callBaseMethod(this,"initialize");
this.raiseEvent("init",Sys.EventArgs.Empty);
J._initializeModulesManager();
...
J._initializeToolProvider();
var K=$get(this.get_id()+"ContentHiddenTextarea");
if(K){this._contentHiddenTextarea=K;
...
var M=this.nodesTracker=new c.Editor.NodesTracker(this);
M.startTracking();
...
J._initMobileEnterFullScreen();
J._initFindReplace();
J._addOrientationListener();
J._setOrientationDecoration();
if(J.get_mode()==c.EditModes.Design){M.convertSingleNbspToTrackedZwsp();
M.addTrackedZwspBeforeInnerLists();
}this.jsHrefStorage=new b.JSHrefStorage();
if(N!==true){this._raiseEditorLoad();
...
}}},_shouldAutoResizeHeight:function(){var J=this;
if(J.get_renderMode()===D.Mobile){return false;
}return J.get_autoResizeHeight()||J._isInInlineEditMode();
},_addInlineEditModeStyles:function(){this._addInlineEditorStyle(r);
},_addInlineEditorSelectedStyles:function(){this._addInlineEditorStyle(s);
},_addInlineEditorStyle:function(J){var K=this;
var L=K.get_element();
a(L).addClass(J);
},_removeInlineEditorSelectedStyles:function(){var J=this;
var K=J.get_element();
a(K).removeClass(s);
},_postponeResizeIframeHeight:function(){setTimeout(Function.createDelegate(this,function(){this._resizeContentArea();
}),300);
},_commandExecutingHandler:function(K,J){if(!K.get_enableImmutableElements()){return;
}var L=K.getSelectedElement();
if(L&&c.Editor.Utils.isImmutableNode(L)){return J.set_cancel(true);
}},_commandExecutedHandler:function(K,J){if(!K.get_enableImmutableElements()){return;
}if(/^(?:Un|Re)do$/.test(J.get_commandName())){c.Editor.Utils.restoreImmutableElements(K);
c.Editor.Utils.revertOriginalImmutables(K);
}},_initializeImmutableElements:function(){var J=this;
if(!J.get_enableImmutableElements()){return;
}c.Editor.Utils.getImmutableElements(this).each(function(K,L){J.setNodeImmutability(L);
});
},setNodeImmutability:function(L){if(!this.get_enableImmutableElements()){return;
}L.setAttribute("contentEditable",false);
$telerik.addCssClasses(L,["reUnselectable"]);
a(L).find("*").addBack().attr("unselectable","on");
var K=this._immutableElements;
if(!K){K=this._immutableElements={};
}var J=new Date().getTime();
if(K[J.toString()]&&K[J.toString()]!=L){while(K[J.toString()]){J++;
}}J=J.toString();
L.setAttribute(c.Editor.Utils.immutableElementId,J);
K[J]=L;
},removeNodeImmutability:function(K){var J=K.getAttribute(c.Editor.Utils.immutableElementId);
K.removeAttribute("contentEditable");
this._cleanUnselectableAttributes(K);
K.removeAttribute(c.Editor.Utils.immutableElementId);
if(!J||!this._immutableElements||!this._immutableElements[J]){return;
}delete this._immutableElements[J];
},_cleanUnselectableAttributes:function(J){$telerik.removeCssClasses(J,["reUnselectable"]);
if(!J.className){J.removeAttribute("class");
}a(J).find("*").addBack().removeAttr("unselectable");
...
this.add_modeChange(a.proxy(function(K,J){if(K.get_mode()==2&&this._elementsResizableWidget){this._disposeElementResizableWidget();
}},this));
this.add_submit(a.proxy(function(){if(this._elementsResizableWidget){this._disposeElementResizableWidget();
}},this));
},_elementResizingSelectionChangeHandler:function(){var K=this;
var L=K._getElementForResizing(K.getSelectedElement());
K._onDocumentUpDelegate=K._onDocumentUpDelegate||Function.createDelegate(K,K._resizingOnUpHandler);
if(L&&!c.Editor.Utils.isImmutableNode(L)){if(K._elementsResizableWidget&&K._elementsResizableWidget._element===L){K._elementsResizableWidget.repaint();
K._elementsResizableWidget.trigger(E,{});
K._elementsResizableWidget._toggleDocumentCursor();
}else{K._disposeElementResizableWidget();
this._elementsResizableWidget=this._getResizable(L);
var J=new Telerik.Web.UI.Editor.GenericCommand("Resize "+L.tagName.toLowerCase(),this.get_contentWindow(),this);
K._elementsResizableWidget.add_resizeStart(function(M){K.get_contentArea().removeAttribute("contenteditable");
if(K.get_document()!=i){a(i).offEvent(n.Up+j).onEvent(n.Up+j,K._onDocumentUpDelegate);
}});
K._elementsResizableWidget.add_resizeEnd(function(M){K.get_contentArea().setAttribute("contenteditable","true");
K.executeCommand(J);
K._cleanHandlesFromRestorePoint(J);
K._reInitResizable(M);
a(i).offEvent(n.Up+j);
});
K._markHandlesForContentFilter();
}}else{if(this._elementsResizableWidget){this._disposeElementResizableWidget();
}}},_resizingOnUpHandler:function(){var J=this;
if($telerik.isSafari&&I.isTag(J.getSelectedElement(),"img")){setTimeout(function(){J._elementResizingSelectionChangeHandler();
},50);
}},_getCleanResizableHandlesFilter:function(){var J=this.get_filtersManager()?this.get_filtersManager().getFilterByName("CleanResizableHandles"):null;
if(!J){J=new Telerik.Web.UI.Editor.CleanResizableHandles();
}return J;
},_cleanHandlesFromRestorePoint:function(J){if(!J.RestorePoint2){return;
}var K=i.createElement("div");
K.innerHTML=J.RestorePoint2.HtmlText;
J.RestorePoint2.HtmlText=this._getCleanResizableHandlesFilter().getHtmlContent(K).innerHTML;
},_getResizable:function(J){var K={liveResize:true,useTinyHandles:true,ownerDocument:this.get_document()};
if($telerik.isTouchDevice){K.handleSize=16;
}return new Telerik.Web.UI.Widgets.Resizable(J,K);
...
}},_reInitResizable:function(J){J.dispose();
this._elementsResizableWidget=null;
this._elementResizingSelectionChangeHandler();
},_getElementForResizing:function(L){var K=null,J=L;
if(L&&L.tagName&&L.tagName.toLowerCase()=="img"){return L;
}while(J&&J!=this.get_contentAreaElement()&&J!=$get(this.get_id()+"Center")){if(J.tagName&&J.tagName.toLowerCase()=="table"){K=J;
break;
}J=J.parentNode;
}return K;
},_markHandlesForContentFilter:function(){var K=this._elementsResizableWidget._handlesCollection;
for(var J in K){K[J]._element.setAttribute("rel","rrWidget");
}},_initRightToLeft:function(){if(this._isEditorRightToLeft()){var L=this.get_element();
var M=this.get_contentAreaElement();
Sys.UI.DomElement.addCssClass(L,"reRtl");
var K=M.contentDocument||M.contentWindow?M.contentWindow.document:null;
if(K){this._setElementStyleProperty(K.body,"direction","rtl");
this._setElementStyleProperty(K.body,"textAlign","right");
}}var J=this.get_dialogOpener();
J.set_additionalQueryString(J.get_additionalQueryString()+"&isRtl="+!!this._isEditorRightToLeft());
...
}if(typeof(Sys.WebForms)!="undefined"&&typeof(Sys.WebForms.PageRequestManager)!="undefined"&&this._saveContentDelegate){var N=Sys.WebForms.PageRequestManager.getInstance();
if(N){N.remove_initializeRequest(this._saveContentDelegate);
}N=null;
...
if(J){if(this._formElementsSelection){this._formElementsSelection.disableFormElementsSelecting();
}if(this._textFieldsTyping){this._textFieldsTyping.disableTextFieldsTyping();
}$clearHandlers(J);
try{J.parentNode.removeChild(J);
}catch(L){}J=null;
}try{this._disposeEvents();
}catch(L){}this._fixMozillaDOMProblems(false);
this._fixIEVisibilityProblems(false);
this._makeResizeable(false);
K._onHiddenTextAreaFocusDelegate=null;
this._onToolClickDelegate=null;
...
K._attachEnterFullScreenMobileHandler(false);
K._enterFullScreenMobileDelegate=null;
...
this._accessKeyTextbox=null;
}if(this._immutableElements){for(var M in this._immutableElements){delete this._immutableElements[M];
...
},_applyAccessKey:function(){var J=this.get_element().getAttribute("accessKey");
if(J){this.get_element().removeAttribute("accessKey");
var K=i.createElement("input");
K.setAttribute("name",this.get_id()+"EditorAccessKey");
K.setAttribute("type","text");
K.setAttribute("id",this.get_id()+"EditorAccessKey");
K.setAttribute("accessKey",J);
this._accessKeyTextbox=K;
$addHandlers(this._accessKeyTextbox,{focus:this.setFocus},this);
var L=i.createElement("div");
L.style.width="0px";
L.style.height="0px";
L.style.overflow="hidden";
L.appendChild(this._accessKeyTextbox);
this.get_element().appendChild(L);
}},_resizeContentArea:function(){this._sizer.resizeIframeHeight();
},_makeResizeable:function(J){if(this._renderMode==c.RenderMode.Classic){this._removeCurrentResizeExtender();
if(!J){return;
...
}var K={se:this._bottomResizer};
this._resizeExtender=new c.ResizeExtender(this,this.get_element(),K,this._tableElement);
...
var L={liveResize:true,enableFrameOverlay:true,handles:[{direction:"SE",element:this._view.resizerHandle()}],constraints:this._initialConstraints};
this._resizableWidget=new Telerik.Web.UI.Widgets.Resizable(this.get_element(),L);
...
}}},_calcInitialConstraints:function(){var K=30;
var L=Math.max(a(this._view.zone("Left")).outerHeight(),a(this._view.zone("Right")).outerHeight());
var J=a(this._view.modulesContainer()).outerHeight()+a(this._view.modesRowContainer()).outerHeight()+a(this._view.toolBarContainer()).outerHeight();
return{minWidth:Math.ceil(a(this._element).innerWidth()),minHeight:Math.ceil(Math.max(L,J)+K)};
},_setSizingConstraints:function(){var L=this,M=L.get_element(),J=a(M),N=L._resizableWidget;
if(N){var K=this._initialConstraints||{minWidth:J.outerWidth(),minHeight:J.outerHeight()};
N.set_constraints(K);
...
},_fixIEVisibilityProblems:function(K){if(typeof(this.get_element().onpropertychange)=="undefined"){return;
}if(K){var J=this._getInvisibleParent();
if(J){this._onIEParentVisibilityChangeDelegate=Function.createDelegate(this,this._onIEParentVisibilityChange);
this._invisibleParent=J;
...
}}},_fixMozillaDOMProblems:function(L){var J=this.get_element();
if(J.onpropertychange||J.onpropertychange===null){return;
}if(L){var K=this._getInvisibleParent();
if(K){this._invisibleParent=K;
this._onMozillaParentVisibilityChangeDelegate=Function.createDelegate(this,this._onMozillaParentVisibilityChange);
K.addEventListener("DOMAttrModified",this._onMozillaParentVisibilityChangeDelegate,false);
...
}}},_onIEParentVisibilityChange:function(K){K=K.rawEvent;
if(!K){return;
}if(K.propertyName=="style.display"||K.propertyName=="className"){var J=$telerik.getCurrentStyle(this._invisibleParent,"display");
if(J!="none"){this.raiseEvent("firstShow");
this.onParentNodeChanged();
this._fixIEVisibilityProblems(false);
}}},_onMozillaParentVisibilityChange:function(J){if(J.attrName=="style"||J.attrName=="class"){var K=J.target;
if((J.currentTarget==J.target)&&"none"!=$telerik.getCurrentStyle(K,"display")){window.setTimeout(Function.createDelegate(this,function(){if(!this._editorFirstVisible){this._editorFirstVisible=true;
this.raiseEvent("firstShow");
this.onParentNodeChanged();
}if(this.get_mode()!=c.EditModes.Preview){this.set_editable(true);
}else{this.set_editable(false);
}}),0);
}}},_onMozillaParentNodeChanged:function(J){if(!J.target||!this.get_element()){return;
}var K=$telerik.isDescendantOrSelf(J.target,this.get_element());
if(K){if($telerik.isFirefox){this.onParentNodeChanged();
}else{window.setTimeout(Function.createDelegate(this,this.onParentNodeChanged),0);
}}},onParentNodeChanged:function(){var K=this.get_renderMode()===D.Mobile&&!this.isFullScreen();
...
if(!K){var L=this.get_mode();
this.set_mode(L);
}this._initContentAreaHtml(J,true);
this.reAttachExternalEvents();
this.set_html(J,null,false,false);
this._enableFormElementsSelecting();
this._enableTextFieldsTyping();
this.whitespaceTracker.startTracking();
this.nodesTracker.startTracking();
if(K){this._attachEnterFullScreenMobileHandler(false);
this._initMobileEnterFullScreen();
}},onResizeStart:function(){var J=this.get_element();
this.resizeStartPosition={position:J.style.position,top:J.style.top,left:J.style.left};
this._resizeSelection=this.getSelection().getRange();
this._sizer.onResizeStart();
},onResizeEnd:function(){if(this.resizeStartPosition){var J=this.get_element();
$telerik.$(J).css({position:this.resizeStartPosition.position||"",top:this.resizeStartPosition.top||"",left:this.resizeStartPosition.left||""});
...
if(this._resizeSelection){var K=null;
if(this._resizeSelection.commonParentElement){K=this._resizeSelection.commonParentElement();
}else{if(this._resizeSelection.parentElement){K=this._resizeSelection.parentElement();
}}if(K&&(this.get_document()==K.ownerDocument)){this.getSelection().selectRange(this._resizeSelection);
}this._resizeSelection=null;
}},_onEditorResizing:function(){if(this._sizer&&this._sizer.onResizing){this._sizer.onResizing();
}},_onResizable:function(K,J){this._sizer.onResizing();
this.get_toolAdapter().repaint();
this.refreshContentZoom();
},_onResizableEnd:function(K,J){this.get_toolAdapter().repaint(true);
this.refreshContentZoom();
...
},getSelectedElement:function(S){var M=this;
var L=M.get_contentArea();
if(M.get_renderMode()===D.Mobile){var P=a(L).find(".__reTextHighlight,.__reNodeHighlight");
if(P.length){var O=P.get(0);
var R=P.last().get(0);
return O===R?O:I.findCommonAncestor(O,R);
}}S=S||M.getDomRange();
if(!S){var J;
try{J=M.get_document().activeElement;
}catch(N){return H;
}return a(J).is("input[type=text],input[type=password],textarea")?J:H;
}var K=S.commonAncestorContainer;
if(K===L||a.contains(L,K)){var T=b.RangeEdges.commonNode(S);
var Q=I.isTextNode(T);
return Q||(S.isCollapsed()&&!Q&&!I.canHaveChildren(T))?T.parentNode:T;
...
},getEditorSelectionRange:function(){var J=this.getSelection().getRange();
var K=this.getSelectedElement();
if(J!=null&&K!=null&&K.ownerDocument!=this.get_document()){J=null;
}return J;
},getDomRange:function(){var K=this,M=K.getSelection(),L=M.getRange(true),J=b.DomRange.toDomRange(L);
return J;
},collapseRange:function(K){var J=this.getDomRange();
J.collapse(K);
J.select();
},selectElement:function(J,K){if(c.Editor.Utils.selectElement(this.get_contentWindow(),J)&&false!=K){this.raiseEvent("selectionChange",Sys.EventArgs.Empty);
}},selectElementContents:function(N,O){var M=this;
var R=M.getDomRange();
var K=M.get_document();
if(e.ie&&K.selection&&!K.getSelection){return this.selectElement(N,O);
}try{R.selectNodeContents(N);
R.select();
}catch(L){var J=R.commonAncestorContainer;
var S=function(U){return t(J,U);
};
if(S("table")){R.selectNode(N);
}else{if(S("tr")||S("tbody")||S("thead")){var T=J.getElementsByTagName("td");
var P=T[0];
var Q=T[T.length-1];
R.setStart(P,0);
R.setEnd(Q,Q.childNodes.length);
}}R.select();
}if(O!==false){this.raiseEvent("selectionChange",Sys.EventArgs.Empty);
}},undo:function(J){this._commandsManager.undo(J);
},redo:function(J){this._commandsManager.redo(J);
},executeCommand:function(L,M,J){var K=(this.get_renderMode()===D.Mobile);
if(!L.get_window()){L.set_window(this.get_contentWindow());
}if(false!=M&&!this.isOpera){this.setFocus();
}var N=L.get_title();
if(K){this._executeCommandEvent("commandExecuting",N);
}this._commandsManager.execute(L,J);
if(K){this._executeCommandEvent("commandExecuted",N);
}},executeBrowserCommand:function(K,J,P,L){if((this.get_enableTrackChanges()||this.get_enableComments())&&K.indexOf("Justify")==0){var M=this.getSelection();
var N=M.getParentElement();
if(N){if(N.getAttribute("Name")!=null){if(N.getAttribute("Name").indexOf("_wp")==0){return;
}}}}var O=this.getLocalizedString(K);
this.executeCommand(new c.Editor.BrowserCommand(O,this.get_contentWindow(),K,P,this));
this.setActive();
this.setFocus();
if(true==L){this.raiseEvent("selectionChange",Sys.EventArgs.Empty);
}},createElement:function(P,N,M,K,L,O){var J=this.get_document().createElement(P);
if(N){J.style.width=N;
}if(M){J.style.height=M;
}if(null!=K){J.id=K;
}if(null!=L){J.name=L;
}if(null!=O){J.value=O;
}return J;
},createRestorePoint:function(){var K=this;
var M=K.get_contentWindow();
var J=K.get_contentArea();
var L=new b.RestorePoint(M,J);
a.observable(L);
L.on({restored:K._onContentRestored},K);
return L;
},_onContentRestored:function(J){var K=this;
var N=K.getDomRange();
K.findContent("div.rrHandle").remove();
var Q=I.getAllChildNodesBy(K.get_contentArea(),function(S){return S.nodeType===3;
});
var R=K.nodesTracker;
var O=/^[\u00A0]$/;
var P=/[\u200b]/;
var M=function(S){return I.isTextNode(S)&&O.test(S.nodeValue)&&I.isSingleChild(S)&&!I.isTableCell(S.parentNode);
};
Array.forEach(Q,function(S){if(P.test(S.nodeValue)){R.trackNode(S);
}else{if(M(S)){R.convertSingleNbspToTrackedZwsp(S);
}}});
var L=I.isZwspTextNode(N.commonAncestorContainer);
R.update(N,L);
if(!e.ie){N.select();
}},getToolState:function(M,K){switch(M){case"Undo":return this.get_commandsManager().isUndoAvailable()?c.Editor.CommandStates.Off:c.Editor.CommandStates.Disabled;
...
case"MobileEdit":var L=this.get_toolAdapter();
if(L&&L.isEditEnabled){return L.isEditEnabled()?c.Editor.CommandStates.On:c.Editor.CommandStates.Off;
}else{return c.Editor.CommandStates.Off;
}break;
default:var J=c.Editor.UpdateCommandsArray[M];
if(J&&J.getState){return J.getState(this.get_contentWindow(),this,K);
}}},getToolValue:function(L,K){var J=c.Editor.UpdateCommandsArray[L];
if(J&&J.getValue){if(J.set_editor){J.set_editor(this);
}return J.getValue(this.get_contentWindow(),K);
}},_addFilters:function(){var J=this;
var K=this.get_filtersManager();
K.clear();
var L=this.usesContentFilter("ConvertToXhtml");
K.set_enableXhtmlFilter(L);
if(this.usesContentFilter("ConvertInlineStylesToAttributes")){K.add(new c.Editor.ConvertInlineStylesToAttributesFilter());
}else{if(L){K.add(new c.Editor.CleanAttributesFilter());
if($telerik.isFirefox||$telerik.isSafari){K.add(new c.Editor.MozillaRemoveLastBrFilter());
}}}J._addEnabledFilter("RemoveScripts",new c.Editor.StripScriptsFilter());
J._addEnabledFilter("EncodeScripts",new c.Editor.EncodeScriptsFilter());
J._addEnabledFilter("FixUlBoldItalic",new c.Editor.FixUlBoldItalic());
K.add(new c.Editor.FixNestedLists());
J._addEnabledFilter("StripCssExpressions",new c.Editor.StripCssExpressionsFilter());
J._addEnabledFilter("StripDomEventAttributes",new c.Editor.StripDomEventAttributesFilter());
J._addEnabledFilter("ConvertFontToSpan",new c.Editor.ConvertFontToSpanFilter());
J._addEnabledFilter("OptimizeSpans",new c.Editor.OptimizeSpans());
J._addEnabledFilter("FixEnclosingP",new c.Editor.FixEnclosingP());
J._addEnabledFilter("ConvertCharactersToEntities",new c.Editor.ConvertCharactersToEntities());
if($telerik.isIE){K.add(new c.Editor.IEKeepObjectDesignFilter());
K.add(new c.Editor.IEKeepObjectParamsFilter());
K.add(new c.Editor.IEKeepCommentsFilter());
K.add(new c.Editor.IEFixEmptyParagraphs());
J._addEnabledFilter("IECleanAnchors",new c.Editor.IECleanAnchorsFilter());
}if(!$telerik.isIE&&!$telerik.isOpera){if(!$telerik.isSafari){K.add(new c.Editor.MozillaKeepFlash(this.get_mozillaFlashOverlayImage()));
}else{K.add(new c.Editor.FixParagraphsWithSingleBrFilter());
}K.add(new c.Editor.MozillaKeepStylesString());
K.add(new c.Editor.MozillaKeepStylesDom());
J._addEnabledFilter("MozEmStrong",new c.Editor.MozEmStrongFilter());
}J._addEnabledFilter("ConvertTags",new c.Editor.ConvertTagsFilter());
if($telerik.isOpera||$telerik.isSafari||$telerik.isChrome||$telerik.isSpartan){K.add(new c.Editor.CleanResizableHandles());
}K.add(new c.Editor.StripJunkFilter());
J._addEnabledFilter("RemoveExtraBreaks",new c.Editor.RemoveExtraBreaks());
K.add(new b.MetaTagAttributesFilter());
J._addEnabledFilter("MakeUrlsAbsolute",new c.Editor.MakeUrlsAbsolute());
J._addEnabledFilter("IndentHTMLContent",new c.Editor.IndentHTMLContentFilter());
J._addEnabledFilter("PdfExportFilter",new c.Editor.PdfExportFilter());
},_addEnabledFilter:function(K,J){if(this.usesContentFilter(K)){this.get_filtersManager().add(J);
}},applyFiltersForEmbedContent:function(L){var M=L+"";
var J=this.get_filtersManager().getFilterByName("IEKeepObjectParamsFilter");
var K=this.get_filtersManager().getFilterByName("MozillaKeepFlash");
if(J){M=J.getDesignContent(M);
}if(K){var N=this.get_document().createElement("div");
Telerik.Web.UI.Editor.Utils.setElementInnerHtml(N,M);
N=K.getDesignContent(N);
M=N.innerHTML;
}return M;
},usesContentFilter:function(J){if(typeof(J)==="string"){J=c.EditorFilters[J];
}var K=J&&!!(this.get_contentFilters()&J);
return K;
},addShortCut:function(J,K){if(this._shortCutManager){this._shortCutManager.addShortCut(J,K);
}},removeShortCut:function(J){if(this._shortCutManager){this._shortCutManager.removeShortCut(J);
}},setShortCut:function(J,K){if(this._shortCutManager){this._shortCutManager.setShortCut(J,K);
}},_addDefaultShortCuts:function(){var K="CTRL",N="F11",M="F10";
if(z&&z.mac){K="CMD";
N=K+"+F11";
M=K+"+F10";
}var L=[["SelectAll",K+"+A"],["Copy",K+"+C"],["Paste",K+"+V"],["Cut",K+"+X"],["Bold",K+"+B"],["Italic",K+"+I"],["Underline",K+"+U"],["Copy",K+"+INS"],["Paste",K+"SHIFT+INS"],["ToggleScreenMode",N],["LinkManager",K+"+K"],["ImageManager",K+"+G"],["SetToolFocus",M]];
if(this.get_enableUndoRedo()){L.push(["Undo",K+"+Z"]);
L.push(["Redo",K+"+Y"]);
}for(var J=0;
J<L.length;
J++){this.addShortCut(L[J][0],L[J][1]);
...
this.addShortCut("InsertParagraph",K+"+ENTER");
}else{if(this.isFirefox){this.addShortCut("EnterParagraphMozilla",K+"+ENTER");
}else{if($telerik.isSafari){this.addShortCut("InsertParagraphSafari",K+"+ENTER");
...
},attachEventHandler:function(J,K){this._attachCashableEvent(J,K,this._contentAreaExternalEventHandlers);
},attachInternalHandler:function(J,K){this._attachCashableEvent(J,K,this._contentAreaEventHandlers);
},_attachCashableEvent:function(K,L,J){if(K.startsWith("on")){K=K.replace("on","");
}this._attachExternalHandler(K,L);
if(!J[K]){J[K]=[];
}J[K].push(L);
},_attachExternalHandler:function(K,L){var J=this.get_contentAreaMode()==c.EditorContentAreaMode.Div?this.get_contentArea():this.get_document();
$telerik.addExternalHandler(J,K,L);
},reAttachExternalEvents:function(){var K=this._contentAreaExternalEventHandlers;
for(var J in K){var L=K[J];
for(var M=0;
M<L.length;
M++){this._attachExternalHandler(J,L[M]);
}}},detachEventHandler:function(K,L){if(K.startsWith("on")){K=K.replace("on","");
}var J=this.get_contentAreaMode()==c.EditorContentAreaMode.Div?this.get_contentArea():this.get_document();
$telerik.removeExternalHandler(J,K,L);
},_getContentElement:function(){var K=this;
if(K.get_contentAreaMode()==l.Div){return K.get_contentArea();
}else{var J=K.get_document();
return J.documentElement||J;
}},_disposeEvents:function(){var K=this;
var J=a(K._getContentElement());
...
},_detachCashableEvents:function(K){for(var M in K){var J=K[M];
if(J.length!=null){for(var O=0;
O<J.length;
O++){var N=J[O];
if(typeof(N)=="function"){try{this.detachEventHandler(M,N);
}catch(L){}}}}}},get_TopZone:function(){return $get(this.get_id()+"Top");
},_createTools:function(){var J=this;
if(!J._onToolClickDelegate){J._onToolClickDelegate=Function.createDelegate(J,J._onToolClick);
}J._createHeaderTools();
if(!J.get_toolAdapter()){var L=J.get_toolContainer();
var K=eval(J.get_toolAdapterType());
J.set_toolAdapter($create(K,{editor:J,toolJSON:J.get_toolJSON(),contentAreaMode:J.get_contentAreaMode()},{toolClick:J._onToolClickDelegate},null,L));
}},_createHeaderTools:function(){var L=this;
if(!L.get_headerToolsToolAdapter()){var M=L.get_headerToolsJSON();
if(M.length){var J=eval(L.get_headerToolsToolAdapterType());
var K={editor:L,headerToolsJSON:M};
var N={toolClick:L._onToolClickDelegate};
L.set_headerToolsToolAdapter($create(J,K,N,null,L.get_headerToolsContainer()));
}}},_onToolClick:function(K,J){if((this.get_enableTrackChanges()||this.get_enableComments())&&K._name=="Cut"&&J._commandName=="Cut"){if(!this._canEditSelection()){alert(this.get_localization()["caneditselection"]);
return false;
}}this.fire(K.get_name(),J);
},getContextMenuByTagName:function(J){var K=this.get_toolAdapter();
if(K&&K.getContextMenuByTagName){return K.getContextMenuByTagName(J);
}},updateTools:function(J,K){if(J&&J.get_tools){Array.forEach(J.get_tools(),function(L){K(L);
});
}},getToolByName:function(K){var J=this;
return J._getTool(J.get_toolAdapter(),K);
},getHeaderToolByName:function(K){var J=this;
return J._getTool(J.get_headerToolsToolAdapter(),K);
},_getTool:function(J,K){if(J&&J.getToolByName){return J.getToolByName(K);
}return null;
},getLocalizedString:function(K,J){if(!K){return J;
}var L=this.get_localization()[K];
if(null==L){L=this.get_localization()[K.toLowerCase()];
}return L;
},zoomContent:function(K){var J=this.get_contentArea();
var M=J.style;
var L=M.zoom=K||"100%";
if((e.ff||e.operaPresto)&&L.indexOf("%")!=-1){L=parseInt(L,10)/100;
}if(e.ff){M.MozTransform=(L==1)?"":String.format("translate({0}px, {1}px) scale({2})",J.offsetWidth*((L-1)/2),J.offsetHeight*((L-1)/2),L);
}if(e.operaPresto){var N=(L==1)?"":String.format("translate({0}px, {1}px) scale({2})",J.offsetWidth*((L-1)/2.04),J.offsetHeight*((L-1)),L);
M.OTransform=N;
}},refreshContentZoom:function(){var J=this.get_contentArea().style.zoom;
this.zoomContent(J);
},setFocus:function(){var M=this;
var P=M._renderMode==c.RenderMode.Mobile;
var O=M.get_mode()==c.EditModes.Html;
var N=M.get_contentAreaMode()==c.EditorContentAreaMode.Div;
if(P&&!O){return;
}try{if($telerik.isIE&&M.getSelection().isControl()&&!M._isInInlineEditMode()){return;
}var K=O?M.get_textArea():M.get_contentWindow();
if(N){K=M.get_contentArea();
}if(K&&K.focus){if($telerik.isIE){var J=M.get_document().activeElement;
if(J&&J==K){return;
}}if(!M._isInInlineEditMode()&&!(K==M.get_textArea()&&!K.offsetHeight)){if(e.edge){if(!O){K=M.get_contentArea();
}K.setActive();
}else{K.focus();
}}}}catch(L){}},focusFirstText:function(){var J=this.domRangeToFirstText();
J.select();
},domRangeToFirstText:function(){var K=this;
var J=K.get_contentArea();
var L=J.children.length?I.firstTextChild(J.children[0]):I.firstTextChild(J);
var M=b.DomRange.createRange(K.get_document());
M.setStart(L,0);
M.setEnd(L,0);
return M;
...
}var J=this._getCurrentArea();
if(J&&J.setActive&&J.isContentEditable!==false){J.setActive();
...
var J=this.get_document().activeElement;
var K=I.isAncestorOf(L,J);
if(L&&L.setActive&&!K&&L.isContentEditable!==false){L.setActive();
}},_getCurrentArea:function(){var J=this;
return J.get_mode()==c.EditModes.Html?J.get_textArea():J.get_contentArea();
},set_visible:function(K){var J=this.get_element();
if(J){J.style.display=K?"":"none";
}this.set_editable(K);
},enableEditing:function(M,K,Q){var J=this.get_toolAdapter();
var L=function(){};
if(!K){K=c.EditingOptions.All;
}this._editingOptions=K;
this._optionalIgnoredTools=Q;
if(this.usesEditingOption("ContextMenus")){if(J){J.enableContextMenus(M);
}}if(this.usesEditingOption("EditModes")){if(!M){this._originalEditModeRef=this.set_mode;
this.set_mode=L;
}else{if(this._originalEditModeRef){this.set_mode=this._originalEditModeRef;
this._originalEditModeRef=null;
}}}if(this.usesEditingOption("Tools")){if(J){if(!M){J.setToolState(null,c.Editor.CommandStates.Disabled);
if(Q){var N=[];
for(var O in Q){var R=J.getToolByName(O);
if(R){N[N.length]=R;
}}J.setToolState(N);
}this._originalSetToolStateRef=J.setToolState;
J.setToolState=L;
}else{if(this._originalSetToolStateRef){J.setToolState=this._originalSetToolStateRef;
this._originalSetToolStateRef=null;
}}}}if(this.usesEditingOption("Modules")){var P=this._modulesManager;
if(P){P.setModulesVisible(M);
}}if(this.usesEditingOption("Typing")){if(!M){this._disableTypingDelegate=$telerik.cancelRawEvent;
...
}}}if(this.usesEditingOption("Tab")){if(!M){this.removeShortCut("InsertTab");
}else{this.setShortCut("InsertTab","TAB");
}}if(M){if(J){J.setToolState(null,c.Editor.CommandStates.Off);
}this.raiseEvent("selectionChange",Sys.EventArgs.Empty);
}},usesEditingOption:function(J){if(typeof(J)==="string"){J=c.EditingOptions[J];
}return J&&!!(J&this._editingOptions);
},set_editable:function(J){var L=this;
if(e.ie||e.operaPresto||this.get_contentAreaMode()==c.EditorContentAreaMode.Div){window.setTimeout(function(){try{L.get_contentArea().setAttribute("contentEditable",""+J);
if(e.ie){L.get_document().execCommand("2D-Position",false,true);
}}catch(M){}},0);
}else{var K=function(){try{L.get_document()["designMode"]=J?"on":"off";
if(J){L.get_document().execCommand("UseCSS",false,true);
L.get_document().execCommand("styleWithCSS",false,false);
}}catch(M){}};
K();
}},get_editable:function(){if(e.ie||e.operaPresto||this.get_contentAreaMode()==c.EditorContentAreaMode.Div){var J=this.get_contentArea().getAttribute("contentEditable",2);
return typeof J==="boolean"?J:J=="true";
}else{return(this.get_document()["designMode"]=="on");
}},enableContentArea:function(J){if(this.isIE){this.get_contentArea().setAttribute("contentEditable",""+J);
}},showExternalDialog:function(W,J,X,P,M,L,V,Q,K,S,T,N){var R=this.getDomRange();
var O=this;
var U=function(Z,Y){window.setTimeout(function(){if(R){try{R.select();
}catch(aa){}}if(M&&Y!=null){var ab=M(Z,Y);
if(false==ab){return;
}O.raiseEvent("selectionChange",Sys.EventArgs.Empty);
}O.setFocus();
},50);
};
this.get_dialogOpener().openUrl(W,J,X,P,U,L,V,Q,K,S,T,N);
},showDialog:function(L,J,K){var O=this.getDomRange();
var N=this;
var Q=function(U,T){window.setTimeout(function(){if(O&&L!="FindAndReplace"){try{O.select();
}catch(V){}}if(K&&T!=null){var W=K(U,T);
if(false==W){return;
}N.raiseEvent("selectionChange",Sys.EventArgs.Empty);
}N.setFocus();
},50);
};
if(e.iemobile){var R=this.get_toolAdapter();
if(R.hideKeyboard){R.hideKeyboard();
}}var M=this.get_dialogOpener();
if(L=="InsertLink"||L=="InsertImage"||L=="InsertTableLight"){var P=this.get_skin();
if(P=="Bootstrap"){J.height=220;
J.width=370;
J.stripPopupHeight=true;
}if(P=="Glow"||P=="Silk"){J.height=230;
J.width=370;
J.stripPopupHeight=true;
}if(this.get_isSkinTouch()){J.height=287;
J.width=386;
}M.openLight(L,J,Q);
}else{M.open(L,J,Q);
}if(this.isIE){var S=this.get_dialogOpener()._getDialogContainer(L);
if(!S||S._DRAG_ATTACHED){return;
}S._DRAG_ATTACHED=true;
S.add_dragEnd(function(){if(O){try{O.select();
}catch(T){}}});
}},_getModeButtonsWrapper:function(){return this._view?this._view.modeButtonsContainer():null;
},_registerModeChangeHandler:function(J){var K=this._getModeButtonsWrapper();
if(!K){return;
}if(J){this._onModeButtonClickDelegate=Function.createDelegate(this,this._onModeButtonClick);
$addHandler(K,"click",this._onModeButtonClickDelegate);
}else{if(this._onModeButtonClickDelegate){$removeHandler(K,"click",this._onModeButtonClickDelegate);
this._onModeButtonClickDelegate=null;
}}},_onModeButtonClick:function(J){var K=this._view.getModeFromEvent(J)||this.get_mode();
if(K!=this.get_mode()){this.set_mode(K);
}return $telerik.cancelRawEvent(J);
},_changeModeUI:function(J){this._view.changeMode(J);
},_isEditModeEnabled:function(J){return J&this.get_editModes()?true:false;
},get_mode:function(){return this._mode;
},set_mode:function(N){if(!this.validateMaxHtmlLength()){this.displayMaxHtmlLengthErrorMessage();
return false;
...
var O=(this._mode==K.Design);
this.set_editable(O);
this.toggleEnhancedEdit(O);
this._setEditableDelegate=null;
}});
this.add_editReady(this._setEditableDelegate);
var M=this._mode;
var K=c.EditModes;
if(N!=K.Design&&this._emptyMessageContainer){this._hideEmptyMessage();
}this.whitespaceTracker.reset();
this.nodesTracker.reset();
if(N==K.Preview&&M!=K.Html){I.setTargetsForPreview(this);
}else{if(M==K.Preview){I.restoreTargetsAfterPreview(this);
}}if(M==K.Html||N==K.Html){if(N==K.Html&&this.get_enableImmutableElements()){I.replaceImmutableElements(this);
}var L=this.get_html(true);
this._mode=N;
this._showTextArea(this._mode==K.Html);
this.set_html(L);
if(N==K.Preview){I.setTargetsForPreview(this);
}if(N!=K.Html&&this.get_enableImmutableElements()){I.restoreImmutableElements(this);
}}else{this._mode=N;
this._showTextArea(this._mode==K.Html);
}this._changeModeUI(N);
if(this._mode!=K.Preview){this.setFocus();
if(!(this.isFirefox&&(!this.get_document()||!this.get_contentArea()))){this.set_editable(true);
}if(this._mode==K.Design){this.toggleEnhancedEdit(true);
}}else{this.set_editable(false);
this.toggleEnhancedEdit(false);
}if(N==K.Design&&$telerik.isFirefox){this.get_contentAreaElement().blur();
}else{this.setFocus();
}var J=this.get_contentAreaElement();
if(I.isTag(J,"iframe")){a(J)[this._mode==K.Html?"addClass":"removeClass"]("reHtmlMode");
}this.raiseEvent("modeChange",Sys.EventArgs.Empty);
if(this._sizer){this._sizer.modeChanged(N);
}if(this._resizableWidget){this._setSizingConstraints();
}if(N==K.Html){this.setFocus();
}},get_enableImmutableElements:function(){return this._enableImmutableElements;
},set_enableImmutableElements:function(L){var K=this._enableImmutableElements;
L=!!L;
if(K==L){return;
}if(!this.get_contentArea||!this.get_contentArea()){this._enableImmutableElements=L;
return;
}if(L){this._enableImmutableElements=L;
this._initializeImmutableElements();
}else{if(this._immutableElements){var J=this;
c.Editor.Utils.getImmutableElements(J).each(function(N,M){J.removeNodeImmutability(M);
});
this._enableImmutableElements=L;
}}},_getHtmlModeTextarea:function(){var L=this;
if(!L._htmlModeTextarea){var K=L.get_contentAreaElement();
var M=L._htmlModeTextarea=K.ownerDocument.createElement("textarea");
var J=a(M);
J.addClass("reTextArea reTextAreaHidden").appendTo(K.parentNode);
if(this.get_renderMode()==c.RenderMode.Mobile){J.addClass("t-flex");
}}return L._htmlModeTextarea;
},get_textArea:function(){return this._getHtmlModeTextarea();
},_showTextArea:function(N){var K=this;
var J=K.get_contentAreaElement();
var L=K._getHtmlModeTextarea();
var M=K.get_contentAreaMode()==c.EditorContentAreaMode.Div;
if(N){var O=K._sizer;
if(($telerik.isSafari||$telerik.isIE9)&&!M){K.hideViewElement(J);
}else{O.hideDesignViewElement(J);
}a(L).removeClass("reTextAreaHidden");
}else{if(($telerik.isSafari||$telerik.isIE9)&&!M){K.showViewElement(J);
}else{J.style.display="";
}a(L).addClass("reTextAreaHidden");
}if(K._isInInlineEditMode()){K._setShowToolbarHandlerToHiddenTextArea(L,N);
}},_setShowToolbarHandlerToHiddenTextArea:function(K,L){var J=this;
if(K){if(L){J._onHiddenTextAreaFocusDelegate=A(J._onHiddenTextAreaFocus,J);
a(K).onEvent(o,J._onHiddenTextAreaFocusDelegate);
}else{a(K).offEvent(o,J._onHiddenTextAreaFocusDelegate);
J._onHiddenTextAreaFocusDelegate=null;
}}},_onHiddenTextAreaFocus:function(J){var K=this;
var L=K.get_toolAdapter();
J.stopPropagation();
L._showToolbarHolder(true);
K._addInlineEditorSelectedStyles();
},hideViewElement:function(J){J.style.width="0px";
J.style.height="0px";
},showViewElement:function(J){J.style.width="100%";
J.style.height="2px";
},_setContentAsText:function(K){var J=this.get_textArea();
if(J){J.value=K;
}},_getContentAsText:function(){var J=this.get_textArea();
if(J&&J.value){var K=J.value;
return K;
}return"";
},set_html:function(K,O,N,M){var L=c.EditModes;
if(this.get_mode()!=L.Html){var J=new c.Editor.GenericCommand(O?O:"Set HTML",this.get_contentWindow(),this);
this._initContentAreaHtml(K);
this.executeCommand(J,N);
if(false!=M){this.raiseEvent("selectionChange",Sys.EventArgs.Empty);
}if(this.get_emptyMessage()&&(!K||K.trim().length==0)&&this.get_mode()==L.Design){this._setEmptyMessage();
c.Editor.CommandList.SetToolFocus("SetToolFocus",this,null);
}}else{this._setContentAsText(K);
}},get_html:function(V,ad){var P=null;
var M="";
var J;
var R=this;
var ae=R.whitespaceTracker;
var ab=R.nodesTracker;
var Z=R.get_mode();
var aa=c.EditModes;
var W=R.get_fullPage();
var Q=R.get_document();
var S=R.get_enableImmutableElements();
if(Z==aa.Preview){I.restoreTargetsAfterPreview(R);
ae.reset();
ab.reset();
}if(Z==aa.Html){var T=R._getContentAsText();
T=R._cleanZeroWidthSpace?T.replace(B,""):T;
return V?R._filterContent(T):T;
}if(S){J=I.getImmutableElements(R);
if(J.length){if(!R._immutableElements){R._initializeImmutableElements();
}I.replaceImmutableElements(R);
}}ae.removeZeroWidthNodes();
ab.removeZeroWidthNodes();
if(W){if(Q){P=Q.getElementsByTagName("HTML")[0];
P=I.cloneNodeWithChildren(P);
var K=R._getAllSheets(P);
for(var U=0;
U<K.length;
U++){var X=K[U];
var Y=X.getAttribute("id");
if(Y&&Y.indexOf("RADEDITORSTYLESHEET")==0){X.parentNode.removeChild(X);
}}if(e.ie||$telerik.isOpera){var L=P.getElementsByTagName("BODY")[0];
L.removeAttribute("contentEditable");
}}}else{var N=R.get_contentArea();
if(e.edge){var O=i.createElement("body");
I.setElementInnerHtml(O,N.innerHTML);
P=V&&N?O:N;
}else{P=V&&N?I.cloneNodeWithChildren(N):N;
}}if(P){if(ad&&(c.EditorStripHtmlOptions&&ad)==c.EditorStripHtmlOptions.Comments){var ac=new c.Editor.StripCommentsFilter();
P=ac.getHtmlContent(P);
}if(V){M=R.get_filtersManager().getHtmlContent(P);
}else{M=W?$telerik.getOuterHtml(P):P.innerHTML;
}ab.restoreZeroWidthNodes();
ae.restoreZeroWidthNodes();
}if(W&&R._doctypeString){M=R._doctypeString+"\n"+M;
}if(S&&J&&J.length){M=I.setImmutablesHtml(M,R);
I.restoreImmutableElements(R);
}M=R._cleanZeroWidthSpace?M.replace(B,""):M;
M=V?M.trim():M;
return M;
},_filterContent:function(P){var N=this;
var S=N._isFullPage(P);
N.set_fullPage(S);
if(S){N._doctypeString=N._extractDoctype(P);
}var O=this.get_filtersManager();
P=O.getDesignContent(P);
var K=N.get_contentAreaElement();
var M=K.ownerDocument;
if(S){var Q=M.createElement("iframe");
var J=a(Q).insertAfter(K).hide();
var R=Q.contentWindow.document;
R.open();
R.write(P);
R.close();
P=O.getHtmlContent(R.body.parentNode);
P=N._doctypeString+"\n"+P;
J.remove();
}else{var L=M.createElement("div");
I.setElementInnerHtml(L,P);
a(L).addClass("reContentArea");
P=O.getHtmlContent(L);
}return P.trim();
},_isFullPage:function(J){return J&&J.toLowerCase().indexOf("<html")!=-1;
},set_initialContent:function(J){if(J!=null){this._initialContent=J;
}else{this._initialContent=this.get_html(true);
}},get_initialContent:function(){return this._initialContent;
},get_text:function(N){var K=this;
var L="";
var M=K.get_mode()==c.EditModes.Html;
var J=M?K.get_textArea():K.get_contentArea();
if(J){L=J[M?"value":"innerHTML"];
}var O=b.Utils._stripAllExceptNewLines(K,L);
O=O.replace(/<\/?[^>]*>/ig,"\n");
O=I._convertEntitiesToCharacters(O);
if(N){if(N.trimText){O=O.replace(/(^\s+)|(\s+$)/g,"");
}if(N.removeMultipleSpaces){O=O.replace(/ +/g," ");
}}return O;
},_getUniqueStyleSheetId:function(J){return"RADEDITORSTYLESHEET"+J;
},_getAllSheets:function(L){if(!L){alert(this.get_localization()["getallsheetscalledwithnodocumenterror"]);
}var K=L.getElementsByTagName("link");
var N=L.getElementsByTagName("style");
var J=[];
var O;
for(O=0;
K[O];
O++){var M=K[O].rel?K[O].rel:K[O].getAttribute("rel");
if(typeof(M)=="string"&&M.toLowerCase().indexOf("style")+1){Array.add(J,K[O]);
}}for(O=0;
N[O];
O++){Array.add(J,N[O]);
}return J;
},_getClassName:function(L){var M=L&&L.selectorText?L.selectorText:"";
var K=M.lastIndexOf(".");
if(K==-1){return"";
}var J=M.indexOf(" ",K);
if(-1==J){J=M.indexOf(":",K);
}if(-1==J){J=M.length;
}return M.substring((K+1),J);
},_createCssFilterObject:function(){if(this._cssClassesHash){return;
}var J=this.get_cssClasses();
if(J&&J.length>0){var M={};
var L=J.length;
for(var K=0;
K<L;
K++){var N=J[K][0];
var O=J[K][1];
M[N.replace(/(.*?)\./ig,function(P){return P.toUpperCase();
})]=O;
}this._cssClassesHash=M;
}},_getFilteredCssClasses:function(M){this._createCssFilterObject();
if(M&&this._cssClassesHash){var J=[];
var L=M.length;
for(var K=0;
K<L;
K++){var O=M[K];
var N=this._checkCssFilter(O[1].selectorText);
if(N){O[2]=N;
J[J.length]=O;
}}return J;
}else{return M;
}},_checkCssFilter:function(J){if(!J||!this._cssClassesHash){return null;
}return this._cssClassesHash[J.replace(/(.*?)\./ig,function(K){return K.toUpperCase();
})];
},getCssArray:function(O){var M=this.get_document();
var L=[];
for(var K=0;
K<M.styleSheets.length;
K++){var N=M.styleSheets[K];
try{this._getStyleSheetRules(L,N,O);
}catch(J){}}L=this._getFilteredCssClasses(L);
return L;
},_getStyleSheetRules:function(N,V,W){var L=V.href;
if(L&&L.indexOf("WebResource.axd?")>-1){return;
}if(L&&L.indexOf(this.get_contentAreaCssFile())>-1){return;
}if($telerik.isIE&&V.imports){for(var P=0;
P<V.imports.length;
P++){this._getStyleSheetRules(N,V.imports[P],W);
}}var J;
try{J=(V.rules)?V.rules:V.cssRules;
}catch(M){return;
}var K=J&&J.length?J.length:0;
for(var O=0;
O<K;
O++){var S=J[O];
if(S&&S.cssText){var X=S.cssText.toLowerCase();
if(X.indexOf("@import")>=0&&S.parentStyleSheet&&S.styleSheet&&S.parentStyleSheet!=S.styleSheet){this._getStyleSheetRules(N,S.styleSheet,W);
continue;
}}var U=S&&S.selectorText?S.selectorText:"";
var Q=U.lastIndexOf(".");
if(Q<0){continue;
}Q=U.lastIndexOf(":");
if(Q>0){continue;
}var T=S.selectorText.split(".")[0];
if(W&&T&&T!==""&&T.toLowerCase()!==W.toLowerCase()){continue;
}var R=this._getClassName(S);
if(R!="reContentArea"){N[N.length]=[R,S];
}}},copyStyleSheets:function(X,aa){if(null==X&&null==aa){return;
}var K=0;
var ab=null;
if(aa.styleSheets.length==0){if(aa.createStyleSheet){aa.createStyleSheet();
}else{var L=aa.createElement("style");
L.media="all";
L.type="text/css";
var T=aa.getElementsByTagName("head")[0];
T.appendChild(L);
ab=L;
}}if(aa.styleSheets[0]){ab=aa.styleSheets[0];
}for(var O=0;
O<X.styleSheets.length;
O++){try{var Y=X.styleSheets[O];
var M=Y.href;
var R=false;
if($telerik.isFirefox){if(Y.ownerNode&&Y.ownerNode.tagName.toLowerCase()=="style"){R=true;
}}if(M&&!R){continue;
}if(Y.media){var Q=Y.media;
if(Q.mediaText!=null){Q=Q.mediaText;
}if(Q&&Q!="all"&&Q!="screen"){continue;
}}var J=(Y.rules)?Y.rules:Y.cssRules;
for(var P=0;
P<J.length;
P++){var V=J[P];
if(ab.addRule){var W=V.selectorText;
var S=V.style.cssText;
if(S&&W){ab.addRule(W,S,K);
}}else{if(ab.insertRule){ab.insertRule(V.cssText,K);
}else{var Z=V.selectorText+"{"+V.style.cssText+"}";
var U=aa.createTextNode(Z);
ab.appendChild(U);
}}K++;
}}catch(N){}}},_contentAreaCommonInitialization:function(){var J=c.Editor.PopupController;
if(J){J.detachFromDocument(this.get_document());
J.attachToDocument(this.get_document());
}this._initEvents();
...
var J=this.get_document().body;
this.set_contentArea(J);
this.get_filtersManager().getDesignContentDom(J);
var Q=0;
c.Editor.Utils.addStyleSheet(this.get_contentAreaCssFile(),this.get_document(),this._getUniqueStyleSheetId(Q++));
var S=this.get_tableLayoutCssFile();
if(S){c.Editor.Utils.addStyleSheet(S,this.get_document(),this._getUniqueStyleSheetId(Q++));
}var K=this.get_cssFiles(),N,R;
if(K.length>0){for(N=0;
N<K.length;
N++){R=this._getUniqueStyleSheetId(Q++);
c.Editor.Utils.addStyleSheet(K[N],this.get_document(),R);
}}else{if(!this.get_fullPage()){this.copyStyleSheets(i,this.get_document());
var P=this._getAllSheets(i);
for(N=0;
N<P.length;
N++){R=this._getUniqueStyleSheetId(Q++);
var O=P[N];
if(O.tagName=="LINK"){var L=O.href;
if(!L||L.indexOf("WebResource.axd?")>-1){continue;
}if(O.rel&&O.rel.toLowerCase()!="stylesheet"){continue;
}if(O.media&&O.media!="all"&&O.media!="screen"){continue;
}c.Editor.Utils.addStyleSheet(O.getAttribute("href"),this.get_document(),R);
}}}}this._contentAreaCommonInitialization();
}catch(M){}},_extractDoctype:function(J){var K=new RegExp("(<!DOCTYPE[^>]+>)","g");
var L=J.match(K);
return L?L[0]:"";
},_setEmptyMessage:function(J){if(this.get_mode()!=c.EditModes.Design){return;
}if(J!=null){var L=this.get_contentArea().innerHTML.trim();
if(L&&L!="<br>"&&L!="<p>&nbsp;</p>"&&L!="<br />"){return;
}}var N=this.get_emptyMessage();
if(N){if(!this._emptyMessageContainer){var M=this._emptyMessageContainer=i.createElement("div");
var K=this.get_contentAreaElement();
K.parentNode.insertBefore(M,K);
M.style.zIndex=K.style.zIndex+1;
M.style.position="absolute";
$telerik.addCssClasses(M,["reEmptyMessage"]);
if($telerik.isIE&&!$telerik.isIE9Mode){M.style.filter="alpha(opacity=40)";
}else{M.style.opacity="0.4";
}c.Editor.Utils.setElementInnerHtml(M,N);
this.add_selectionChange(Function.createDelegate(this,this._hideEmptyMessageOnSelectionChange));
$addHandlers(M,{mousedown:this._hideEmptyMessageOnMouseDown},this);
$addHandlers(K,{blur:this._setEmptyMessage},this);
}else{this._emptyMessageContainer.style.display="";
}}},_hideEmptyMessageOnSelectionChange:function(J){this._hideEmptyMessage();
},_hideEmptyMessageOnMouseDown:function(J){if(this._emptyMessageContainer){if(J.rawEvent.preventDefault){J.rawEvent.preventDefault();
}J.returnValue=false;
this.raiseEvent("selectionChange",Sys.EventArgs.Empty);
var K=this;
setTimeout(function(){K.setFocus();
},0);
return false;
}},_hideEmptyMessage:function(){this._emptyMessageContainer.style.display="none";
},_initContentAreaHtml:function(M,L){if(!M||M.trim().length==0){this._setEmptyMessage();
}var N=this.get_filtersManager().getDesignContent(M);
var O=null;
if(this._isFullPage(N)){this.set_fullPage(true);
O=N;
this._doctypeString=this._extractDoctype(O);
...
N=I.stripHtmlHeadBodyTags(N);
this.set_fullPage(false);
}N=this._markExistingEmptyParagraphs(N);
if(this.get_contentAreaMode()==c.EditorContentAreaMode.Div){this.set_contentArea(this.get_contentAreaElement());
var J=this.get_contentArea();
this.set_document(J.ownerDocument);
this.set_contentWindow(window);
if(N.match(/<form[\s>]/i)){alert(this.get_localization()["formelementsaresupportedbyiframeerror"]);
N="";
}this._setContentAreaHtml(J,N);
if(!this._divContentAreaInitialized){this._contentAreaCommonInitialization();
}}else{if(O||L){this._recreateIframe(O,N);
}else{this._setContentAreaHtml(this.get_contentArea(),N);
}}var K=this._checkForBrokenParagraphs();
if(K){this._fixBrokenParagraphs(N);
}},_recreateIframe:function(P,N){P=P||($telerik.isSafari?"<!DOCTYPE html><html><head><style></style></head><body></body></html>":"<head><style></style></head><body"+($telerik.isOpera?" style='height:100%'":"")+"></body>");
var M=this;
var J=M.get_contentAreaElement();
if($telerik.isFirefox){var O=Function.createDelegate(M,function(){if(this._execLoadFunctionSecondTime){this._setContentAreaHtml(this.get_contentArea(),N);
this._contentFrameLoadFunction();
}});
$addHandler(J,"load",O);
}try{var K=J.contentWindow.document;
K.open();
K.write(P);
K.close();
M._execLoadFunctionSecondTime=false;
if(K.body){I.setElementInnerHtml(K.body,I.stripHtmlHeadBodyTags(N));
M._contentFrameLoadFunction();
}else{M._execLoadFunctionSecondTime=true;
}}catch(L){}},_setContentAreaHtml:function(J,K){I.setElementInnerHtml(J,K);
this.get_filtersManager().getDesignContentDom(J);
},_markExistingEmptyParagraphs:function(J){var K=J+"";
K=K.replace(/<p><\/p>|<p([^>]+)><\/p>|<p[^\/+]\/>/gi,'<p$1 _rdEditor_exists="1">&nbsp;</p>');
return K;
},_checkForBrokenParagraphs:function(){var M=false;
var L=this.get_contentArea().getElementsByTagName("p");
for(var J=L.length-1;
J>=0;
J--){var K=L[J];
if(!K.hasChildNodes()){if(!K.getAttribute("_rdEditor_exists")){M=true;
}}K.removeAttribute("_rdEditor_exists");
}return M;
},_fixBrokenParagraphs:function(J){if(this.get_contentAreaMode()!=c.EditorContentAreaMode.Div){J=Telerik.Web.UI.Editor.Utils.stripHtmlHeadBodyTags(J);
}var K=this.get_contentArea();
J=J.replace(/<p ([^>]+)>|<p>/ig,'<div $1 _rdEditor_temp="1">');
J=J.replace(/<\/p>/gi,"</div>");
c.Editor.Utils.setElementInnerHtml(K,J);
var O=K.getElementsByTagName("DIV");
for(var L=O.length-1;
L>=0;
L--){var N=O[L];
if(N.getAttribute("_rdEditor_temp")){var P=this.get_document().createElement("P");
$telerik.getOuterHtml(N.cloneNode(false)).replace(/([a-z0-9\-_]+)=/gi,function(S,R){var T;
if(R.toLowerCase()!=="_rdeditor_temp"){T=N.getAttribute(R);
if(!T&&R==="class"){T=N.className;
}P.setAttribute(R,T);
}});
for(var M=0;
M<N.childNodes.length;
M++){P.appendChild(N.childNodes[M].cloneNode(true));
}if(N.swapNode){N.swapNode(P);
}else{var Q=N.parentNode;
Q.insertBefore(P,N);
Q.removeChild(N);
}}}this.get_filtersManager().getDesignContentDom(K);
},_createUI:function(){var N=this;
if(this._created){return;
}this._registerModeChangeHandler(true);
this._tableElement=this.get_mainTable();
var J=$get(this.get_id()+"Center");
var K=this.get_contentAreaElement();
if(J){if($telerik.isMobileSafari&&this.get_contentAreaMode()==c.EditorContentAreaMode.Iframe){var L=i.createElement("div");
a(L).addClass("reIframeWrapperIOS");
L.appendChild(K);
J.appendChild(L);
this._contentAreaWrapper=L;
}else{J.appendChild(K);
}}else{var M=a(N.get_element()).find(f);
M.append(K);
var O=N.get_findReplaceOverlay();
if(O){a(O.get_element()).insertBefore(M);
}}this._created=true;
},repaint:function(){c.RadEditor.callBaseMethod(this,"repaint");
var J=this.get_toolAdapter();
J.repaint(true);
this._sizer.onRepaint(this.isVisible());
},_updateEditorSize:function(J,K){this.repaint();
},_fixMoreSizeIssues:function(){this.repaint();
},_setElementStyleProperty:function(J,K,L){J.style[K]=L;
},setSize:function(K,J){this._sizer.setEditorSize(K,J);
this.refreshContentZoom();
},pasteHtml:function(O,N,M,L,K){if(!this.get_editable()){return;
}this._disposeElementResizableWidget();
if(O.match(/^<script/ig)){O="<span id='_rdEditor_temp'>&nbsp;</span>"+O;
}var J=new c.EditorCommandEventArgs(N,null,O);
if(false==this._executeCommandEvent("pasteHtml",N,J)){return;
}if(O!=J.get_value()){O=J.get_value();
}var P=this.getLocalizedString(N);
var Q=this.get_mode();
if(c.EditModes.Design==Q){this.setFocus();
this._fireDeleteSelectionCmd(true);
var U=new c.Editor.PasteHtmlCommand(P,this.get_contentWindow(),O,M,this);
this.executeCommand(U,null,K);
if(L!=false){this.raiseEvent("selectionChange",Sys.EventArgs.Empty);
}}else{if(c.EditModes.Html==Q){var W=this.get_textArea();
if(this.isIE&&this.get_document().selection){W.setActive();
var V=i.selection.createRange();
V.text=O;
}else{if(W.setSelectionRange){var T=W.selectionStart;
var S=W.selectionEnd;
var R=O;
W.value=W.value.substring(0,T)+R+W.value.substring(S);
W.setSelectionRange(T+R.length,T+R.length);
this.setFocus();
}}}}a("#_rdEditor_temp",this.get_document()).remove();
},_nonEditableCommands:["XhtmlValidator","Copy","Print","SelectAll","AboutDialog","Help","Zoom"],_isCommandEditable:function(J){return Array.indexOf(this._nonEditableCommands,J)==-1;
},fire:function(K,J){var N=this;
if(!N.get_editable()&&this._editorFinishedLoad&&N._isCommandEditable(K)){return;
}if(K!="Copy"){this._saveTypedContent(true,"editor.fire Copy executing");
}this.setActive();
if(false==this._executeCommandEvent("commandExecuting",K,J)){return;
}if(this._optionalIgnoredTools&&!this._optionalIgnoredTools[K]){return;
}this._pendingTextTypeCmd=null;
var L=c.Editor.CommandList[K];
var O=false;
if(L){O=(false!=L(K,this,J));
}else{alert("The command "+K+" is not implemented yet.");
}if(O){if(!$telerik.isOpera){this.setFocus();
}this.raiseEvent("selectionChange",Sys.EventArgs.Empty);
}this._executeCommandEvent("commandExecuted",K,J);
try{if(K=="Undo"&&!$telerik.isIE&&this.get_contentArea().innerHTML.toLowerCase().trim()=="<br>"){this.get_contentArea().innerHTML="<br>";
}}catch(M){}},_executeCommandEvent:function(L,K,J){if(!J){J=new c.EditorCommandEventArgs(K);
}this.raiseEvent(L,J);
return !(J.get_cancel&&J.get_cancel());
},_onWindowResize:function(){var J=this._getViewportBounds();
this.setSize(J.width,J.height,false);
},_registerWindowResizeHandler:function(J){if(J){this._onWindowResizeDelegate=Function.createDelegate(this,this._onWindowResize);
$addHandler(window,"resize",this._onWindowResizeDelegate);
}else{if(this._onWindowResizeDelegate){$removeHandler(window,"resize",this._onWindowResizeDelegate);
this._onWindowResizeDelegate=null;
}}},_getViewportBounds:function(){var J=$telerik.getClientBounds();
var K=i.documentElement.scrollLeft||i.body.scrollLeft;
var L=i.documentElement.scrollTop||i.body.scrollTop;
J.scrollLeft=K;
J.scrollTop=L;
return J;
},_handleParentsWithOverflow:function(P){var J,O;
if(false==P){if(!this._parentsWithOverflow){return;
}J=this._parentsWithOverflow;
var L=J.length;
for(var K=0;
K<L;
K++){var M=J[K];
...
while(N&&N.tagName!="BODY"){O=N.style;
J[J.length]=[N,O.overflow,O.height,O.tableLayout,O.position];
if(O.position==="fixed"){O.position="static";
}O.tableLayout="auto";
O.overflow="visible";
O.height="auto";
N=N.parentNode;
}}},isFullScreen:function(){return this._isFullScreen;
},_disableBodyScroll:function(){var J=i.body;
this._preFullScreenBodyStyle=[J.scroll,J.style.margin,""];
J.scroll="no";
J.style.margin="0px";
if(J){this._preFullScreenBodyStyle[2]=J.style.overflow;
J.style.overflow="hidden";
}this._handleParentsWithOverflow(true);
},_enableBodyScroll:function(){var J=i.body;
if(!this._preFullScreenBodyStyle){return;
}J.scroll=this._preFullScreenBodyStyle[0];
try{J.style.margin=this._preFullScreenBodyStyle[1];
}catch(K){}J.style.overflow=this._preFullScreenBodyStyle[2];
this._handleParentsWithOverflow(false);
this._preFullScreenBodyStyle=null;
},_setToolOn:function(M,K){var J=this;
var L=J.getToolByName(M);
if(L){L.setOn(K);
}L=J.getHeaderToolByName(M);
if(L){L.setOn(K);
}},toggleScreenMode:function(){var K=this;
var Q=e.operaPresto||z.ios||z.android;
if(K.get_renderMode()==c.RenderMode.Mobile){K._executeToggleScreenModeCmd();
K._setToolOn("ToggleScreenMode",K._isFullScreen);
K.raiseEvent("toggleScreenMode",Sys.EventArgs.Empty);
K.focusFirstText();
if(K._elementsResizableWidget){K._disposeElementResizableWidget();
}return;
}var M=K.get_element();
if(!K._isFullScreen){this._disableBodyScroll();
K._isFullScreen=true;
d(M,p);
var N=M.style.height;
if(N=="auto"||isNaN(parseInt(N,10))){N=M.offsetHeight+"px";
}var O=M.style.width;
if(O=="auto"||isNaN(parseInt(O,10))){O=M.offsetWidth+"px";
}K._preFullScreenBounds={width:O,height:N};
K._onWindowResize();
if(Q){M.style.position="fixed";
M.style.left="0px";
M.style.top="0px";
}else{K._preFullScreenBrowserRect=K._getViewportBounds();
var L=$telerik.getLocation(M);
setTimeout(function(){c.Editor.Utils.restoreBrowserPosition(L.x,L.y);
},0);
}K._registerWindowResizeHandler(true);
if(this._resizeExtender){this._resizeExtender.enable(false);
$telerik.setVisible(this._bottomResizer,false);
}}else{K._registerWindowResizeHandler(false);
this._enableBodyScroll();
K._isFullScreen=false;
C(M,p);
var J=K._preFullScreenBounds;
K.setSize(J.width,J.height,false);
if(Q){M.style.position="";
M.style.left="";
M.style.top="";
}else{var P=K._preFullScreenBrowserRect;
c.Editor.Utils.restoreBrowserPosition(P.scrollLeft,P.scrollTop);
}if(this._resizeExtender){this._resizeExtender.enable(true);
$telerik.setVisible(this._bottomResizer,true);
}}K.set_editable(true);
if(!z.ios){K.setFocus();
}K.refreshContentZoom();
K._setToolOn("ToggleScreenMode",K._isFullScreen);
this.raiseEvent("toggleScreenMode",Sys.EventArgs.Empty);
},validateMaxTextLength:function(M){var L=this.get_maxTextLength();
if(L>0){if(!M){M="";
}var J=this.get_text({removeMultipleSpaces:true})+M;
var K=b.Utils._getCharactersCount(J);
return L>=K;
}return true;
},validateMaxHtmlLength:function(J){var K=this.get_maxHtmlLength();
if(K>0){if(!J){J="";
}return K>=this.get_html(true).length+J.length;
}return true;
...
}},set_contentHiddenTextareaValue:function(J){this._contentHiddenTextarea.value=c.Editor.Utils.encodePostbackContent(J);
},get_localization:function(){return this._localization;
},_createIframe:function(){var J=i.createElement("iframe");
J.frameBorder="0";
...
K.margin="0px";
K.padding="0px";
if(this.get_renderMode()==c.RenderMode.Mobile){J.className="t-flex";
}return J;
...
}if(!this._contentAreaElement){var J;
if(this.get_contentAreaMode()==c.EditorContentAreaMode.Div){J=i.createElement("div");
J.setAttribute("id",this.get_id()+"_contentDiv");
J.className="reContentArea";
}else{J=this._createIframe();
if(!$telerik.isIE){J.innerHTML="Your browser does not support inline frames or is currently configured not to display inline frames.";
}J.setAttribute("id",this.get_id()+"_contentIframe");
}J.setAttribute("title","Rich text editor with ID "+this.get_id());
this._contentAreaElement=J;
if(this.get_enableAriaSupport()){this._contentAreaElement.setAttribute("role","textbox");
}}return this._contentAreaElement;
},findContent:function(K){var J=this.get_contentArea();
if(typeof(K)==="function"){return a(I.getAllChildNodesBy(J,K));
}else{return a(this.get_contentArea()).find(K);
...
},set_useClassicDialogs:function(J){this.get_dialogOpener().set_useClassicDialogs(J);
},get_qualifiedSkin:function(){var J=this.get_skin();
return"RadEditor."+J;
},_applyAriaSupport:function(){this.get_element().setAttribute("role","presentation");
var P=this.get_toolContainer();
this._applyAriaForLayoutTables();
var M;
if(P){P.setAttribute("role","toolbar");
var Q=P.getElementsByTagName("ul");
for(M=0;
M<Q.length;
M++){Q[M].setAttribute("role","presentation");
}}var N=this._getModeButtonsWrapper();
N.setAttribute("role","presentation");
var J=N.getElementsByTagName("*");
for(M=0;
M<J.length;
M++){var K=J[M];
var O=K.tagName.toLowerCase()=="a"?"button":"presentation";
K.setAttribute("role",O);
}var L=this._contentHiddenTextarea;
if(L){L.setAttribute("aria-hidden","true");
}},_applyAriaForLayoutTables:function(){var M=this.get_mainTable();
if(M){M.setAttribute("role","presentation");
var P=M.getElementsByTagName("tr");
for(var K=0;
K<P.length;
K++){var O=P[K];
O.setAttribute("role","presentation");
var N=O.getElementsByTagName("td");
for(var L=0;
L<N.length;
L++){N[L].setAttribute("role","presentation");
}}}var J=$get(this.get_id()+"_BottomTable");
if(J){J.setAttribute("role","presentation");
}},_updateHiddenTextarea:function(){var J=this;
var K=J.get_html(true);
if(J.get_mode()==c.EditModes.Preview){I.restoreTargetsAfterPreview(J);
}if(J.get_enableImmutableElements()){var L=new RegExp(" "+I.immutableElementId+"=['\"]?[0-9]{13}['\"]?","gm");
K=K.replace(L,"");
}J.set_contentHiddenTextareaValue(K);
return K;
},_registerClientValidation:function(){if(typeof(Page_ClientValidate)=="function"){var J=Page_ClientValidate;
Page_ClientValidate=Function.createDelegate(this,function(M){var K=this.get_element();
if(K){var L=this._updateHiddenTextarea();
K.value=L;
K.setAttribute("value",L);
}K=null;
return J(M);
});
}},saveContent:function(K,J){if(this._saveContentDelegate){var L=null;
if(!this.validateMaxHtmlLength()){L=1;
}else{if(!this.validateMaxTextLength()){L=2;
}}if(L){if(J){if(J.set_cancel){J.set_cancel(true);
}}else{if(L==1){this.displayMaxHtmlLengthErrorMessage();
}else{if(L==2){this.displayMaxTextLengthErrorMessage();
...
var K=this._contentHiddenTextarea.form;
if(K){$telerik.addExternalHandler(K,"submit",this._saveContentDelegate);
}if(typeof(__doPostBack)!="undefined"){var P=this._saveContentDelegate;
var N=__doPostBack;
__doPostBack=function(S,R){P();
N(S,R);
};
}if(typeof(Sys.WebForms)!="undefined"&&typeof(Sys.WebForms.PageRequestManager)!="undefined"){var M=Sys.WebForms.PageRequestManager.getInstance();
if(M){if(null!=M._originalDoPostBack){var Q=this._saveContentDelegate;
var O=M._originalDoPostBack;
M._originalDoPostBack=function(S,R){Q();
O(S,R);
};
}if(null!=M._onSubmitStatements&&!$telerik.isIE){var L=this;
var J={};
Array.add(M._onSubmitStatements,function(){var R=L.saveContent(null,J);
return(R!=H)?R:true;
});
}else{M.add_initializeRequest(this._saveContentDelegate);
}}M=null;
}K=null;
},toggleEnhancedEdit:function(L){if(this.get_contentAreaMode()==c.EditorContentAreaMode.Div){var J=this.get_contentArea();
if(typeof(L)=="undefined"||null==L){L=(J.className.indexOf("reContentAreaToggle")>-1)?false:true;
}if(L){Sys.UI.DomElement.addCssClass(J,"reContentAreaToggle");
}else{Sys.UI.DomElement.removeCssClass(J,"reContentAreaToggle");
}this.updateToggleTableBorderToolState(L);
return L;
}else{if($telerik.isSafari&&typeof(L)!="undefined"&&false==this.disableContentAreaStylesheet(L)){var K=this;
setTimeout(function(){K.disableContentAreaStylesheet(L);
},200);
}else{return this.disableContentAreaStylesheet(L);
}}},disableContentAreaStylesheet:function(L){var K=this.get_document();
if(!K||!K.getElementById){return false;
}var J=K.getElementById(this._getUniqueStyleSheetId(0));
if(J){if(L==null){L=J.disabled;
}J.disabled=!L;
this.updateToggleTableBorderToolState(L);
return L;
}else{return false;
}},updateToggleTableBorderToolState:function(K){var J=this.getToolByName("ToggleTableBorder");
if(J&&J.setOn){J.setOn(K);
}},pasteHyperLink:function(O,K){var J=this.getSelection().getParentElement();
var N=J?J.parentNode:null;
if(J!=null&&J.tagName=="A"&&($telerik.isFirefox||($telerik.isIE&&J.style.cssText!=""&&!((N.tagName=="FONT"||N.tagName=="SPAN")&&N.children.length==1)))){J.parentNode.removeChild(J);
}else{if($telerik.isSafari&&N&&N.nodeName=="A"){N.parentNode.removeChild(N);
}else{this.fire("Unlink");
}}var L=c.Editor.Utils.getOuterHtml(O);
if($telerik.isIE){L=this._replaceHeadingInsideLink(L);
if(O.getElementsByTagName("P").length==1){L=this.replaceParagraphAndAnchor(L);
var M=J;
while(M){if(M.tagName=="P"){M.parentNode.removeChild(M);
break;
}M=M.parentNode;
}}}L=L.replace(/<br[^>]*><\/a>$/ig,"</a><br />");
this.pasteHtml(L,K);
},replaceParagraphAndAnchor:function(J){var K=J+"";
K=K.replace(/(<A[^>]*>)\s*(<P[^>]*>)/ig,"$2$1");
K=K.replace(/(<\/P[^>]*>)\s*(<\/A[^>]*>)/ig,"$2$1");
return K;
},_replaceHeadingInsideLink:function(J){var K=J+"";
K=K.replace(/(<A[^>]*>)\s*(<h\d{1}[^>]*>)/ig,"$2$1");
K=K.replace(/(<\/h\d{1}[^>]*>)\s*(<\/A[^>]*>)/ig,"$2$1");
return K;
},_saveTypedContent:function(K,J){if(this._pendingTextTypeCmd){this._pendingTextTypeCmd.update();
}if(true!=K){this._pendingTextTypeCmd=null;
}},_attachBodyHandlers:function(L){var J=null;
if(this.get_document()&&this.get_contentArea()){J=this.get_contentArea();
}if(!J){return;
}if(L){this._onResizeStartDelegate=Function.createDelegate(this,this._onResizeStart);
...
$telerik.addExternalHandler(J,"resizestart",this._onResizeStartDelegate);
$telerik.addExternalHandler(J,"resizeend",this._onResizeEndDelegate);
$telerik.addExternalHandler(J,"drop",this._onDropDelegate);
...
$telerik.addExternalHandler(J,"dragend",this._onDragEndDelegate);
$telerik.addExternalHandler(J,"dragstart",this._onDragStartDelegate);
}var K=this._shouldAutoResizeHeight();
if(($telerik.isIE6||$telerik.isIE7||$telerik.isIE8)&&!K){this._onEditorResizeDelegate=Function.createDelegate(this,this._onEditorResizing);
$telerik.$(this.get_element()).on("resize",this._onEditorResizeDelegate);
}this._onPasteDelegate=Function.createDelegate(this,J.setActive&&J.ownerDocument.queryCommandSupported("Paste")?this._onPaste:this._onBeforePaste);
$telerik.addExternalHandler(J,"paste",this._onPasteDelegate);
if(K){this._resizeContentAreaDelegate=Function.createDelegate(this,this._resizeContentArea);
$telerik.addExternalHandler(J,"keyup",this._resizeContentAreaDelegate);
}}else{if(this._onResizeStartDelegate){$telerik.removeExternalHandler(J,"resizestart",this._onResizeStartDelegate);
}if(this._onResizeEndDelegate){$telerik.removeExternalHandler(J,"resizeend",this._onResizeEndDelegate);
}if(this._onDropDelegate){$telerik.removeExternalHandler(J,"drop",this._onDropDelegate);
}if(this._onDragEndDelegate){$telerik.removeExternalHandler(J,"dragend",this._onDragEndDelegate);
}if(this._onDragStartDelegate){$telerik.removeExternalHandler(J,"dragstart",this._onDragStartDelegate);
}if(this._onEditorResizeDelegate){$telerik.$(this.get_element()).off("resize",this._onEditorResizeDelegate);
}if(this._resizeContentAreaDelegate){$telerik.removeExternalHandler(J,"keyup",this._resizeContentAreaDelegate);
}$telerik.removeExternalHandler(J,"paste",this._onPasteDelegate);
...
}},_initEvents:function(){var K=this;
var J=a(K._getContentElement());
if(!this.isIE){this.attachInternalHandler("keypress",h(K,K._onKeyPressed));
if(e.webkit||e.edge){if(K.get_renderMode()==c.RenderMode.Mobile){J.onEvent(n.Down+k,"img",Function.createDelegate(K,K._onImageDown));
J.onEvent(n.Up+k,"img",Function.createDelegate(K,K._onImageUp));
}else{this.attachInternalHandler("click",Function.createDelegate(K,function(M){var N=M.srcElement?M.srcElement:M.target;
if(I.isTag(N,"img")){K.selectElement(N);
}}));
}J.onEvent(n.Down+k,"map",h(K,function(M){var N=K.findContent("img[usemap=#"+M.currentTarget.id+"]");
if(N.length){K.selectElement(N[0]);
}M.preventDefault();
}));
}if(this.isFirefox&&this.get_contentAreaMode()!=c.EditorContentAreaMode.Div){this.attachInternalHandler("click",function(M){if(!K.get_editable()){return;
}window.setTimeout(function(){K.set_editable(true);
K.setFocus();
},100);
});
}}this.attachInternalHandler("keypress",this._preventEnterPropagation);
this.attachInternalHandler("keydown",function(M){if(K.isOpera){return K._onKeyDown(M);
}else{K._onKeyDown(M);
}if($telerik.isSafari){var O=K._shortCutManager.isShortCutHit(M);
if(null!=O){var N=O.get_name();
if(N==="InsertShiftTab"){$telerik.cancelRawEvent(M);
K._focusLastVisibleTool();
}switch(N){case"Bold":case"Italic":case"Underline":$telerik.cancelRawEvent(M);
break;
}}}});
J.onEvent(n.Down+j,function(){K.setActiveIfElementIsNotInContent();
K._saveTypedContent(false,"Saving typed content onmousedown");
var M=K.get_toolAdapter();
if(M.isEditEnabled&&M.isEditEnabled()){K.fire("MobileEdit");
}});
this.attachInternalHandler("keyup",Function.createDelegate(this,this._onKeyUp));
var L=Function.createDelegate(K,function(M){K.raiseEvent("selectionChange",Sys.EventArgs.Empty);
K._resizingOnUpHandler();
});
J.onEvent(n.Up+j,L);
...
},_onImageDown:function(L){var K=this;
var J=a(K._getContentElement());
var M=L.srcElement?L.srcElement:L.target;
K._imageMoveDelegate=K._imageMoveDelegate||Function.createDelegate(K,K._onImageMove);
J.onEvent(n.Move+k,M,K._imageMoveDelegate);
},_onImageUp:function(L){var K=this;
var J=a(K._getContentElement());
if(!K._imageMoved&&K.get_editable()){K.selectElement(L.srcElement?L.srcElement:L.target);
L.preventDefault();
}setTimeout(function(){K._imageMoved=false;
},0);
J.offEvent(n.Move+k);
},_onImageMove:function(J){this._imageMoved=true;
...
},_initMobileEnterFullScreen:function(){var J=this;
J._attachEnterFullScreenMobileHandler(true);
if(J.get_renderMode()==c.RenderMode.Mobile){J.set_editable(false);
}},_attachEnterFullScreenMobileHandler:function(J){var L=this;
if(L.get_renderMode()==c.RenderMode.Mobile){var K=a([L.get_element(),L.get_contentArea()]);
if(J){b.UI.TapEvent.onTap(K,{tap:h(L,L._enterFullScreenMobileHandler),namespace:"reToggleScreenMode"});
}else{b.UI.TapEvent.offTap(K);
}}},_enterFullScreenMobileHandler:function(){var J=this;
J._executeToggleScreenModeCmd();
J._setToolOn("ToggleScreenMode",J._isFullScreen);
J._attachEnterFullScreenMobileHandler(false);
},_executeToggleScreenModeCmd:function(){var J=new b.ToggleScreenMode(this);
return this.executeCommand(J,false,false);
},_toggleMobileEdit:function(){var J=this;
J._hideMobileKeyboard=J._hideMobileKeyboard===true?false:true;
if(J._hideMobileKeyboard){J._showToolBarContainer();
}else{J._hideToolBarContainer();
...
},_initFindReplace:function(){var J=this.get_findReplaceOverlay();
if(!J){return;
}this.findReplaceManager=new b.FindReplaceManager(this,J,{localization:this.get_localization()});
},_addOrientationListener:function(){var J=this;
if(F){J._mediaListenerDelegate=Function.createDelegate(J,J._onOrientationChange);
J._mediaQuery=window.matchMedia(w);
J._mediaQuery.addListener(J._mediaListenerDelegate);
}},_removeOrientationListener:function(){var J=this;
if(J._mediaQuery&&J._mediaListenerDelegate){J._mediaQuery.removeListener(J._mediaListenerDelegate);
J._mediaListenerDelegate=null;
}},_onOrientationChange:function(){var L=window.orientation,J=L==90||L==-90,K;
if(z.ios&&e.safari){K=J?u.Portrait:u.LandScape;
}else{K=J?u.LandScape:u.Portrait;
}this._setOrientationDecoration(K);
this.raiseEvent("orientationChange",new b.OrientationEventArgs(K));
},_setOrientationDecoration:function(J){if(window.orientation===H){return false;
}if(!J){J=this._getOrientation();
}var K=J==u.LandScape?v:x;
a(this.get_element()).removeClass(x).removeClass(v).addClass(K);
},_getOrientation:function(){var J=window.orientation;
return(J==-90||J==90)?u.LandScape:u.Portrait;
},_shouldCancelKeyEvent:function(J){if(!$telerik.isFirefox){return false;
}if(!this.get_enableImmutableElements()){return false;
}if(J.ctrlKey||J.altKey){return false;
}var N=c.Editor.Utils,M=this.getSelection().getRange();
if(!M){return false;
}var K=function(O){return N.isTextNode(O)&&N.isImmutableNode(O);
};
if((M.collapsed&&K(M.commonAncestorContainer))||(K(M.startContainer)||K(M.endContainer))){var L=J.keyCode;
if(J.shiftKey&&L==13){return true;
}if(L==8||L==32||L==46){return true;
}if(!N.isSystemKey(L)&&!N.isCursorMovingKey(L)){return true;
}}return false;
},_onKeyDown:function(M){var N=this;
var U;
delete N._shortcut;
if(N.get_mode()==c.EditModes.Preview||N._isTargetTextElement(M)){return;
}if(N._shouldCancelKeyEvent(M)){return $telerik.cancelRawEvent(M);
}var P=M.keyCode;
var J=N.get_contentAreaMode();
var L=N.get_document();
this._startInlineEditModeTabListener(P);
if(J==c.EditorContentAreaMode.Iframe&&$telerik.isMobileSafari){this.setFocus();
}if(P==8||P==46){if(this._fireDeleteSelectionCmd(true,true)){return $telerik.cancelRawEvent(M);
}if(P==46){if($telerik.isOpera&&$telerik.OperaVersion<11){L.execCommand("Delete");
this.get_contentArea().contentEditable=true;
if(J!=c.EditorContentAreaMode.Div){L.designMode="on";
...
var Q=this.getLocalizedString("Typing");
if(this.get_enableTrackChanges()){Q=Telerik.Web.UI.Editor.TrackChangesCommandStrings.Delete;
}this._pendingTextTypeCmd=new c.Editor.TextTypeCommand(Q,this.get_contentWindow(),this);
this._hasDeleteExecuted=true;
}if(this.isIE&&L.selection){var V=function(){var ac=N.getSelectedElement();
if(ac&&ac.tagName=="EMBED"){ac.setAttribute("hidden","true");
ac.setAttribute("id","FileToDelete");
window.setTimeout(function(){var ad=L.getElementById("FileToDelete");
if(ad.parentNode&&ad.parentNode.removeChild){ad.parentNode.removeChild(ad);
}},100);
...
var T=V();
if(T){return false;
}try{U=L.selection.createRange();
var S=null;
if(U&&U.duplicate){S=U.duplicate();
}if(8==M.keyCode){U.moveStart("character",-1);
}else{U.moveEnd("character",1);
}if(U.parentElement().tagName=="EMBED"){U.select();
V();
if(S&&S.select){S.select();
}}}catch(O){}var W=new c.Editor.Selection(this.get_contentWindow());
if(W&&W.isControl()){var X=W.getRange();
X.execCommand("Delete");
$telerik.cancelRawEvent(M);
}}if(this.get_enableTrackChanges()||this.get_enableComments()){if(P==8||P==46){var ab=this.getToolByName("AddComment");
if(ab){ab.set_enabled(false);
}}}if(!($telerik.isIE&&P==46)){var K=new b.DeleteFix(N);
K.applyFix(M);
}return;
}var aa=M.target?M.target:M.srcElement;
if(aa&&"INPUT"!=aa.tagName){var Y=this._shortCutManager.isShortCutHit(M);
if(null!=Y&&Y.get_name()=="Copy"){this._shortcutHit=false;
return;
}this._shortcutHit=(null!=Y);
if(this._shortcutHit){var Z=N._shortcut=Y.get_name();
this.fire(Z);
}else{if(c.Editor.Utils.isCursorMovingKey(M.keyCode)){if(!this._hasCursorMoved){this._saveTypedContent(false,"Saving typed content before letting the cursor move");
}this._hasCursorMoved=true;
return;
}if(!M.ctrlKey&&!M.altKey&&(32==M.keyCode||13==M.keyCode||!c.Editor.Utils.isSystemKey(M.keyCode))){this._fireDeleteSelectionCmd();
...
this.executeCommand(this._pendingTextTypeCmd);
this.raiseEvent("selectionChange",Sys.EventArgs.Empty);
}}}if(this._shortcutHit){if($telerik.isIE||$telerik.isSafari||e.edge){var R=Y.get_name();
if(this.get_enableTrackChanges()||this.get_enableComments()){if($telerik.isSafari&&R=="Paste"){return;
}if(R=="Enter"){$telerik.cancelRawEvent(M);
return;
}if(R=="Cut"){return;
}}else{if(R=="Cut"||R=="Paste"){return;
}}$telerik.cancelRawEvent(M);
}}}},_startInlineEditModeTabListener:function(K){var J=this;
if(J._isInInlineEditMode()&&K==9&&!J._tabKeyDownFlag){J._tabKeyDownFlag=true;
J._inlineEditModeTabTimeout=window.setTimeout(function(){var L=J.get_toolAdapter();
if(J._tabKeyDownFlag){if(!I.isEditorContentArea(i.activeElement)){L._showToolbarHolder(false);
L._onInlineEditModeBodyClick(i.activeElement);
}J._tabKeyDownFlag=false;
}},100);
}},_clearInlineEditModeTabListener:function(J){if(this._isInInlineEditMode()&&J==9){this._tabKeyDownFlag=false;
}},_fireDeleteSelectionCmd:function(O,K){var M=this;
if(!M.get_editable()){return;
}var N=M.shouldTrackChanges&&M.shouldTrackChanges();
var J=new c.Editor.DeleteSelectionCommand(M,{complete:K});
var L=J.getState(M.get_contentWindow(),M);
if(!N&&(L!==c.Editor.CommandStates.Disabled)){M.executeCommand(J,true,O);
return true;
}return false;
},_preventEnterPropagation:function(J){if(J.keyCode==Sys.UI.Key.enter){if(J.stopPropagation){J.stopPropagation();
}J.cancelBubble=true;
}},_onKeyUp:function(J){this._clearInlineEditModeTabListener(J.keyCode);
this._cleanUpAfterDelete();
if(!this._shortcutHit&&I.isTypingKey(J)){this._normalizeInsertedText();
...
},_onKeyPressed:function(J){var K=this;
if(!K._pendingTextTypeCmd){K.raiseEvent("selectionChange",Sys.EventArgs.Empty);
}var L=K._shortcut;
if(L){delete K._shortcut;
if(L!=g&&L!=y){J.preventDefault();
return false;
}}},_normalizeInsertedText:function(O){var Q=this.getDomRange();
var L=function(S){return I.isTextNode(S)&&!I.isZwspTextNode(S);
};
if(!Q||!Q.isCollapsed()||!L(Q.commonAncestorContainer)){return;
}var J=Q.commonAncestorContainer;
var P=J.previousSibling;
if(L(P)){var R=Q.startOffset;
var K=J.length;
P.data+=J.data;
J.parentNode.removeChild(J);
J=P;
var N=P.length-(R==K?0:K-R);
Q.setStart(P,N);
Q.setEnd(P,N);
Q.select();
}if(O&&O.normalizeRight){var M=J.nextSibling;
if(L(M)){J.data+=M.data;
M.parentNode.removeChild(M);
Q.select();
}}},_focusLastVisibleTool:function(){var J=this.get_toolAdapter().get_tools();
if(this.get_isInAccessibleMode()){this._focusLastAccessibleToolElement(J);
}else{this._focusLastVisibleToolElement(J);
}},_focusLastAccessibleToolElement:function(K){var J=K[K.length-1].get_element();
J.parentNode.childNodes[0].focus();
},_focusLastVisibleToolElement:function(K){var J=this._getLastVisibleToolElement(K);
J.focus();
},_getLastVisibleToolElement:function(L){var K;
for(var J=L.length-1;
J<L.length;
J--){K=L[J].get_element();
if(K.style.display!=="none"){return K;
}}},_onPaste:function(M){var L=this;
var P=L.get_stripFormattingOptions();
if(P==c.StripFormattingOptions.NoneSupressCleanMessage||L._isTargetTextElement(M)){return;
}if(L._pasting){return $telerik.cancelRawEvent(M);
}L._pasting=true;
var K=I.getClipboardAsHtml(L);
if(!K){return;
}var O=L.usesContentFilter("RemoveScripts"),J=I.cleanPastedContent(K,P,L.get_localization()["askwordcleaning"],O,L);
if(J){var N=L.getDomRange();
window.setTimeout(function(){N.select();
L.pasteHtml(J,"Paste");
L._scrollToSelectionEnd();
L._normalizeInsertedText({normalizeRight:true});
delete L._pasting;
},5);
}else{delete L._pasting;
}return $telerik.cancelRawEvent(M);
},_onBeforePaste:function(M){var N=this,L=N.get_document(),S=N.getSelection(),U=N.get_stripFormattingOptions(),T=!(U&c.StripFormattingOptions.AllExceptNewLines||U&c.StripFormattingOptions.All);
if(N._isTargetTextElement(M)||U==c.StripFormattingOptions.NoneSupressCleanMessage){return;
}if(T&&!I.containsHtmlAtClipboard(M)){var O=new b.ClipboardImagesProvider().getImages(M);
if(O.length>0){N._insertBlobImages(O);
return $telerik.cancelRawEvent(M);
}}if(N._pasting){return $telerik.cancelRawEvent(M);
}N._pasting=true;
N._pendingCutCommand=null;
var J=N.get_contentArea(),W=I.getPasteContainer(J),Q=N.getDomRange();
if(L.createRange){var V=L.createRange();
if(V.selectNodeContents){V.selectNodeContents(W);
}S.selectRange(V);
}else{if(W.setActive){W.setActive();
}}var X=M.clipboardData.types;
var P=(X.contains&&X.contains("text/html"))||(X.indexOf&&X.indexOf("text/html"));
if(P){var K=M.clipboardData.getData("text/html");
var R=/<!--StartFragment-->([\s\S]*)<!--EndFragment-->/;
W.innerHTML=K.replace(R,"$1");
this.jsHrefStorage.storeHrefs(W);
W.innerHTML="";
}if(!N.afterPasteDelegate){N.afterPasteDelegate=Function.createDelegate(N,N._onAfterPaste);
}window.setTimeout(function(){N.afterPasteDelegate(Q,W);
},0);
},_onAfterPaste:function(M,O){this.jsHrefStorage.restoreHrefs(O);
var L=this;
var K=O.innerHTML;
if($telerik.isSafari){K=K.replace(/<div[^>]+class=\"telerik_paste_container\"[^>]*><br><\/div>/gi,"<br/>");
K=K.replace(/<div[^>]+class=\"telerik_paste_container\"[^>]*>(.*?)<\/div>/gi,"$1<br/>");
K=K.replace(/<br[/\s]*>$/i,"");
}I.clearPasteContainer(O);
M.select();
var N=L.usesContentFilter("RemoveScripts");
var J=I.cleanPastedContent(K,L.get_stripFormattingOptions(),L.get_localization()["askwordcleaning"],N,L);
L.pasteHtml(J,"Paste");
L._scrollToSelectionEnd();
L._normalizeInsertedText({normalizeRight:true});
delete L._pasting;
},_isTargetTextElement:function(J){return I.isTextElement(J.target?J.target:J.srcElement);
},_insertBlobImages:function(M){var K=this;
var N=K.get_stripFormattingOptions();
var J=K.get_localization()["askwordcleaning"];
var L=function(O){var Q=O.target;
if(/data:image\//.test(Q.src)){var P=I.cleanPastedContent(I.getOuterHtml(Q),N,J,false,K);
K.pasteHtml(P,"Paste",null,false);
K.collapseRange();
K._scrollToSelectionEnd();
}};
a(M).load(L);
},_scrollToSelectionEnd:function(){var J=this;
var K=b.RangeEdges.endNode(J.getDomRange());
if(K){I.scrollTo(K,J);
}},_onResizeStart:function(J){var L=this.getSelectedElement();
if(L==J.srcElement){var K=this.get_localization()["resizecommand"]||"Resize";
this._pendingResizeCmd=new c.Editor.GenericCommand(K,this.get_contentWindow(),this);
}},onResizing:function(J){if($telerik.isMobileSafari&&this._contentAreaWrapper){this._contentAreaWrapper.style.height=J.height+"px";
}this.get_toolAdapter().repaint();
},_onResizeEnd:function(J){if(this._pendingResizeCmd){this.executeCommand(this._pendingResizeCmd);
}this.refreshContentZoom();
},_onDragStart:function(K){var J=this.get_contentWindow().document;
c.Editor.Utils.storeOriginalPathsAndAttributes(this.get_contentArea());
if($telerik.isFirefox){if(this.get_enableImmutableElements()&&K.target&&c.Editor.Utils.isImmutableNode(K.target)){return $telerik.cancelRawEvent(K);
}return;
}var O=this.get_localization()["movecommand"]||"Move";
this._pendingMoveCommand=new c.Editor.GenericCommand(O,this.get_contentArea(),this);
this._startRange=null;
if(!K.ctrlKey&&!K.ctrlLeft){this._startRange=J.selection?J.selection.createRange():J.body.createTextRange();
if(this._startRange.length){var N=J.body.createTextRange();
var L=this._startRange.item(0);
if("IMG"==L.tagName&&"A"==L.parentNode.tagName&&L.parentNode.childNodes.length==1){L=L.parentNode;
}c.Editor.Utils.storeOriginalPathsAndAttributes(L);
try{N.moveToElementText(L);
this._startRange=N;
}catch(M){}}}},_onDragEnd:function(K){var J=this.get_contentWindow().document;
c.Editor.Utils.restoreOriginalPathsAndAttributes(this.get_contentArea());
if($telerik.isFirefox){return;
}if(this._pendingMoveCommand){if(this._startRange){var N=J.selection?J.selection.createRange():J.body.createTextRange();
if(N.length){var O=J.body.createTextRange();
try{O.moveToElementText(N.item(0));
}catch(M){}N=O;
}try{if(N.compareEndPoints&&0!=N.compareEndPoints("StartToStart",this._startRange)&&0!=N.compareEndPoints("EndToEnd",this._startRange)){this._startRange.execCommand("Delete",false,null);
}}catch(L){}}this.executeCommand(this._pendingMoveCommand);
this._pendingMoveCommand=null;
this.raiseEvent("selectionChange",Sys.EventArgs.Empty);
}},_onDrop:function(J){setTimeout($telerik.$.proxy(function(){c.Editor.Utils.restoreOriginalPathsAndAttributes(this.get_contentArea());
},this),0);
this._saveTypedContent(false,"on drop operation executing");
var K=this.get_localization()["Drop"]||"Drop external content";
this._pendingDockCommand=new c.Editor.GenericCommand(K,this.get_contentWindow(),this);
if(this._elementsResizableWidget){window.setTimeout(h(this,this._elementResizingSelectionChangeHandler),0);
}},_onDocumentDragEnd:function(J){if(!this._pendingDockCommand){return;
}this.executeCommand(this._pendingDockCommand);
this._pendingDockCommand=null;
this.raiseEvent("selectionChange",Sys.EventArgs.Empty);
},_trackChangesLoad:function(){var J=this,K=J.getToolByName("EnableTrackChangesOverride");
if(K){K.setOn(J.get_enableTrackChanges()?0:1);
}this.set_canRejectTrackChange(J.set_canAcceptTrackChange());
if(J.get_enableComments()){Telerik.Web.UI.Editor.TrackChanges.initialize(J);
J.attachInternalHandler("onkeydown",function(L){Telerik.Web.UI.Editor.TrackChanges.checkInsertionPoint(J.getSelectedElement(),J);
});
}}};
a.registerControlProperties(c.RadEditor,{ajaxSpellCheckScriptReference:"",spellAllowAddCustom:true,spellCheckJSON:{},enableUndoRedo:true,headerToolsToolAdapter:null,toolAdapter:null,fullPage:null,dialogOpener:null,ribbonBar:null,links:[],colors:["","#000000","#eeece1","#1f497d","#4f81bd","#c0504d","#9bbb59","#8064a2","#4bacc6","#f79646","#ffffff","#7f7f7f","#ddd9c3","#c6d9f0","#dbe5f1","#f2dcdb","#ebf1dd","#e5e0ec","#dbeef3","#fdeada","#f2f2f2","#595959","#c4bd97","#8db3e2","#b8cce4","#e5b9b7","#d7e3bc","#ccc1d9","#b7dde8","#fbd5b5","#d8d8d8","#3f3f3f","#938953","#548dd4","#95b3d7","#d99694","#c3d69b","#b2a2c7","#92cddc","#fac08f","#bfbfbf","#262626","#494429","#17365d","#366092","#953734","#76923c","#5f497a","#31859b","#e36c09","#a5a5a5","#0c0c0c","#1d1b10","#0f243e","#244061","#632423","#4f6128","#3f3151","#205867","#974806","#c00000","#ff0000","#ffc000","#ffff00","#92d050","#00b050","#00b0f0","#0070c0","#002060","#7030a0"],fontNames:["Arial","Courier New","Garamond","Georgia","MS Sans Serif","Segoe UI","Tahoma","Times New Roman","Verdana"],fontSizes:[1,2,3,4,5,6,7],symbols:["&#8364;","&#162;","&#163;","&#165;","&#164;","&#169;","&#174;","&#8482;","&#177;","&ne;","&#8776;","&#8804;","&#8805;","&#247;","&#215;","&#8734;","&#189;","&#188;","&#190;","&#178;","&#179;","&#8240;","&#182;","&#167;","&#945;","&#946;","&#916;","&#181;","&#937;","&#8721;","&#216;","&ang;","&#186;","&#171;","&raquo;","&#183;","&#8226;","&#8224;","&#8225;","&#402;"],realFontSizes:["8px","9px","10px","11px","12px","13px","14px","16px","18px","20px","22px","24px","26px","28px","32px","36px","48px","72px"],snippets:[],cssClasses:[],cssFiles:[],languages:[],paragraphs:[["<p>","Normal"],["<h1>","<h1>Heading 1</h1>"],["<h2>","<h2>Heading 2</h2>"],["<h3>","<h3>Heading 3</h3>"],["<h4>","<h4>Heading 4</h4>"],["<h5>","<h5>Heading 5</h5>"],["<h6>","<h6>Heading 6</h6>"],["<dir>","<dir>Directory List</dir>"],["<menu>","<menu>Menu List</menu>"],["<pre>","<pre>Formatted</pre>"],["<address>","<address>Address</address>"]],formatSets:[],toolsWidth:null,maxTextLength:null,maxHtmlLength:null,toolProviderID:null,autoResizeHeight:false,toolbarMode:c.EditorToolbarMode.Default,editModes:c.EditModes.All,stripFormattingOptions:c.StripFormattingOptions.None,contentFilters:c.EditorFilters.DefaultFilters,newLineMode:c.EditorNewLineModes.P,contentAreaCssFile:"",tableLayoutCssFile:"",mozillaFlashOverlayImage:"FlashManager.gif",toolAdapterType:"",headerToolsToolAdapterType:"",enableResize:true,headerToolsJSON:[],toolJSON:[],modulesJSON:[],contextMenusJSON:[],skin:"Default",language:"en-us",contentWindow:null,document:null,contentArea:null,contentAreaMode:c.EditorContentAreaMode.Iframe,isInAccessibleMode:false,enableAriaSupport:false,enableComments:false,enableTrackChanges:false,emptyMessage:"",isSkinTouch:false,currentFormatting:null,editType:m.Normal,findReplaceOverlay:null});
a.registerControlEvents(c.RadEditor,["firstShow","pasteHtml","editReady","submit","commandExecuted","commandExecuting","selectionChange","domChange","init","load","modeChange","toggleScreenMode","spellCheckLoaded","pasteDom","pasteBlockElement","orientationChange",q]);
c.RadEditor.registerClass("Telerik.Web.UI.RadEditor",c.RadWebControl);
b.OrientationEventArgs=function(J){b.OrientationEventArgs.initializeBase(this);
this._orientation=J;
};
...
b.DomRangeEventArgs=function(J){b.DomRangeEventArgs.initializeBase(this);
this._range=J;
};
...
},_restoreCollapsedRange:function(r,n){var p=n.parentNode;
var k=new b.Consolidator();
var q=n.previousSibling;
var o=n.nextSibling;
if(k.normalizeTextEdge(n,r)){r.collapse();
}else{if(h.isTextNode(o)||(!q&&h.canHaveChildren(o))){r.setStart(o,0);
r.collapse(true);
}else{if(h.isTextNode(q)){r.setStart(q,q.length);
r.collapse(true);
}else{var s=d?j+j:j;
var m=n.ownerDocument.createTextNode(s);
p.insertBefore(m,n);
r.setEnd(m,s.length-1);
r.collapse();
}}}a(n).remove();
try{r.select();
}catch(l){}},_traverseFragments:function(l){var m=new b.RangeFragmentsTraverser(l);
...
var z=this._contextMenus[E];
if("TH"==E&&!z){z=this._contextMenus.TD;
}var A="";
...
}z=this._contextMenus[A];
}if(z&&E=="IMG"&&D&&"true"==D.getAttribute("isflash")){z=null;
}if(!this.isIE&&A=="*"){return;
}if(z){z.set_eventObject(B);
z.show();
if(d.isSafari){try{var H=D.tagName;
if(H=="IMG"||H=="TD"||H=="TR"){C.selectElement(D);
...
var E=B.get_skin();
var A=B.get_contextMenusJSON();
var C={valueSelected:this._onDropDownValueSelectedDelegate,show:this._onDropDownBeforeShowDelegate,hide:this._onDropDownHideDelegate};
for(var D=0;
D<A.length;
D++){var z=A[D];
z.sizetofit=true;
z.parentElement=B.get_contentAreaElement();
z.skin=E;
z._rendermode=Telerik.Web.UI.RenderMode.toString(B.get_renderMode())=="Lite"?"Lightweight":Telerik.Web.UI.RenderMode.toString(B.get_renderMode());
var G=$create(Telerik.Web.UI.EditorContextMenu,z,C,null,document.createElement("span"));
var F=z.tagName?z.tagName:"*";
this._contextMenus[F]=G;
...
}var N=A.get_contentWindow();
var D=l.isWindowDestroyed(N)||!N.document;
if(A.get_mode()===j.Design&&!D){for(var C=0;
C<M;
C++){var H=K[C];
if(H.get_name){var J=H.get_name();
if(H.setState){var G=E;
if(null==G){G=A.getToolState(J,F);
}if(null!=G){H.setState(G);
}}if(H.updateValue){H.updateValue(A.getToolValue(J,F));
}}}}this._rangeState=null;
...
},_onDropDownBeforeShow:function(O,z){var C=this.get_editor();
var D;
var A;
if(d.isIE&&Telerik.Web.UI.EditorSplitButton.isInstanceOfType(O)&&O.isArrowClicked(event?event.srcElement:null)){C.setFocus();
}if(Telerik.Web.UI.EditorToolStrip.isInstanceOfType(O)){this.setToolState(O.get_items());
}if(!C.getSelection().isControl()&&!Telerik.Web.UI.EditorContextMenu.isInstanceOfType(O)){C.enableContentArea(false);
}var Q=O.get_name();
if(Q=="Undo"){var R=C.get_commandsManager().getCommandsToUndo();
R=this._convertCommandsArray(R);
O.set_items(R);
}else{if(Q=="Redo"){var N=C.get_commandsManager().getCommandsToRedo();
N=this._convertCommandsArray(N);
O.set_items(N);
}else{if(Q=="ModuleManager"){var E=C.get_renderMode()==c.RenderMode.Classic;
var I=C.get_modulesManager();
var G=E?"reModule_hidden_icon":"ModuleHiddenIcon";
var K=E?"reModule_visible_icon":"ModuleVisibleIcon";
if(!I){return;
}var J=I.get_modules();
var H=[];
for(D=0;
...
H[D]=[L,P,A];
}O.set_updateHeader(false);
O.set_items(H);
}}}var B=O.get_items();
if(B&&B.length>0){return;
}var M=null;
switch(Q){case"ApplyClass":M=C.getCssArray();
break;
case"FontName":M=Array.clone(C.get_fontNames());
break;
case"FontSize":M=C.get_fontSizes();
break;
case"InsertSymbol":M=C.get_symbols();
if((d.isIE||d.isSafari)&&M&&M.length){for(D=0;
D<M.length;
D++){if(M[D]=="&"){M[D]="&amp;";
}if(M[D]=="<"){M[D]="&lt;";
}}}break;
case"BackColor":case"ForeColor":M=C.get_colors();
break;
case"RealFontSize":M=C.get_realFontSizes();
break;
case"InsertSnippet":M=C.get_snippets();
break;
case"Zoom":M=["10%","20%","50%","100%","150%","200%","300%","500%"];
break;
case"FormatBlock":M=C.get_paragraphs();
break;
case"FormatSets":M=C.get_formatSets();
break;
case"InsertCustomLink":M=C.get_links();
break;
case"AjaxSpellCheck":case"SpellCheck":M=C.get_languages();
break;
}if(M){O.set_items(M);
...
var B=A.get_selectedItem();
if(Telerik.Web.UI.EditorButton.isInstanceOfType(B)){this._raiseEditorEvent(B,null);
...
}var j=Telerik.Web.UI.Editor.DefaultToolAdapter._visibleToolAdapter;
...
c.SizerLightweightAutoResizeHeight.prototype={makeAutoResizeHeight:function(){var f=this,e=f.editor;
f._setHeight(e.get_contentArea(),"auto");
f.resizeIframeHeight(true);
...
f._setHeight(e.get_contentArea(),"");
}};
