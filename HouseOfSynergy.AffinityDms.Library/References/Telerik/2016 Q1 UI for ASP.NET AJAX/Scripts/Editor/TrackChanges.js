...
},_clearTrackedInlineAncestors:function(m,i){var l=this;
var k=l.get_document();
var o=c.DomRange.createRange(k);
var n=k.createTextNode("a");
m.parentNode.replaceChild(n,m);
o.selectNodeContents(n);
o.select();
var j=new c.TrackChangesRemoveFormat(l,{removeTrackedNodes:!i});
j.onExecute();
n.parentNode.replaceChild(m,n);
},_pasteDomHandler:function(l,i){var j=i.get_element(),k=i.get_cursor();
if(l.get_enableTrackChanges()&&l.get_enableTrackChangesOverride()&&f.getTrackedAncestor(k)){var m=!/[<>]/.test(j.innerHTML);
l._clearTrackedInlineAncestors(k,m);
...
n.appendChild(m);
}},_pasteBlockElementHandler:function(j,i){if(j.shouldTrackChanges()){var k=i.get_element();
...
},_keyPressHandler:function(m){var n=this,p=null,l=null,k=null,i=n._shouldIgnoreKey(m)||!n.shouldTrackChanges();
if(i||n.get_mode()!=Telerik.Web.UI.EditModes.Design){return;
}p=n.getSelection().getRange(true);
k=p.parentElement?p.parentElement():p.commonAncestorContainer;
if(n._isElementInTrackedInsNode(k)){return;
}else{l=n.getDomRange();
var j;
var o=h.isZwspTextNode(l.commonAncestorContainer);
if(!l.isCollapsed()&&!o){n._deleteContentWhenTypingInSelection(l);
}if(o){j=l.cloneRange();
l.setStart(l.commonAncestorContainer,l.commonAncestorContainer.length);
l.collapse(true);
l.select();
}n._executeInsertCommand();
if(j){j.select();
...
return true;
}},_deleteContentWithSelection:function(j){var i=this;
i._shouldCollapseSelection=true;
i._executeDeleteCommand(false);
