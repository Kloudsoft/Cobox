...
(function(a,V){var b=Telerik.Web.UI,s="rcuHeader",e="rcuBody",t="radHideButtonText",C="rcuStatusIcon",p="rcuFileName",q="rcuFileSize",w="rcuList",v="rcuLI",x="rcuLIPending",I="rcuLIUploading",D="rcuLISucceeded",n="rcuLIFailed",h="rcuButtons",f="rcuButton",o="rcuFakeButton",B="rcuRemoveButton",i="rcuCancelButton",g="rcuButtonText",F="rcuToolTip",A="rcuProgressBar",y="rcuProgress",E="rcuToggleButton",j="rcuIconCollapse",m="rcuIconExpand",J="rcuTmplStatus",H="rcuUploadedFiles",G="rcuTotalFiles",z="rcuProgressIcon",l="rcuErrorNotification",r="rcuFileWrap",u="rcuFileInput",k="rcuDisabled",S="span",c="button",U="ul",O="li",d="click",P="mouseenter",Q="mouseleave",T="type",M="input",N="label",L=45,W=12,R=" ",K=".";
b.RadCloudUpload.RenderingManager=function(X,Y,Z,aa){this._owner=Z;
this._controlElement=this._owner._element;
this._element=X;
this._localization=Y;
...
this._showEmptyFileListPanel=aa;
};
b.RadCloudUpload.RenderingManager.prototype={initialize:function(){var X=a(this._element);
this._validFilesListWrapper=X.find(".rcuFileList:not(.rcuFailed)").get(0);
this._invalidFilesListWrapper=X.find(".rcuFailed").get(0);
...
},createRow:function(X){var Y;
this._fileInfo=X;
Y=this._renderRow();
this._getValidFilesListElement().appendChild(Y);
this._showInfoPanel();
...
return Y;
},changeRowState:function(ae,af){var X=a(af),ag=b.UploadedFileState,ad=this._localization,Y=ad.Remove,Z=true,ac,ab=I,aa=n;
switch(ae){case ag.InProgress:ab=x;
aa=I;
Z=false;
Y=ad.Cancel;
break;
case ag.Succeeded:ab=I;
aa=D;
this._uploadedFilesCount++;
break;
case ag.UploadFailed:ac=ad.ServerErrorMessage;
this._failedFilesCount++;
this._showInvalidFilesList();
break;
case ag.SizeValidationFailed:if(X.hasClass(x)){ab=x;
}this._failedFilesCount++;
this._showInvalidFilesList();
ac=ad.SizeValidationFailedMessage;
break;
case ag.ExtensionValidationFailed:if(X.hasClass(x)){ab=x;
}this._failedFilesCount++;
this._showInvalidFilesList();
ac=ad.ExtensionValidationFailedMessage;
break;
}X.removeClass(ab);
X.addClass(aa);
if(Z){this._removeProgressBar(X);
this._toggleActionButtonState(X,Y);
}if(ac){af._message=ac;
this._invalidateFileRow(af);
}this._updateUploadingLabel();
this._hideValidFilesList();
},updateRowProgress:function(Z,Y,X){this._checkRowForProgressBar(Z,X);
this._updateProgressBar(Z,Y);
},removeRow:function(X){a(X).remove();
this._hideInfoPanel();
this._hideInvalidFilesList();
this._hideValidFilesList();
this._totalFilesCount--;
this._updateUploadingLabel();
},renderFileInput:function(){var X=this._renderInput(u,"file"),Y=this._renderLabel();
a(Y).append(X);
a(this._controlElement).find(K+r).prepend(Y);
},appendDefaultDropZone:function(X){this._owner.get_view().appendDefaultDropZone(X);
},_initializeEvents:function(){var X=this;
a(X._validFilesListWrapper).on(d,K+f,function(Y){X._onButtonClick(Y);
});
a(X._invalidFilesListWrapper).on(d,K+f,function(Y){X._onButtonClick(Y);
}).on(P,K+o,function(Y){if(X._getEnabled()){X._onErrorButtonMouseEnter(Y);
}}).on(Q,K+o,function(Y){if(X._getEnabled()){X._onErrorButtonMouseLeave(Y);
}});
a(X._element).on(d,K+E,function(Y){X._onToggleButtonClick(Y);
});
...
},_checkRowForProgressBar:function(Y,X){if(a(Y).find(K+A).length===0){this._renderProgressBar(Y,X);
}},_updateProgressBar:function(Y,X){a(Y).find(K+y).css("width",X).text(X);
},_removeProgressBar:function(X){X.find(K+A).remove();
...
},_showElement:function(X,Y){this._owner.get_view()._showElement(X,Y);
},_hideElement:function(X){this._owner.get_view()._hideElement(X);
},_hideListElement:function(X){if(a(X).find("li").length===0){this._hideElement(X);
}},_onButtonClick:function(Y){var aa=Y.currentTarget,Z=a(aa).parents(K+v).get(0),X=this._extractCommandFromClassName(aa,Z);
this.trigger(X,Z);
},_onErrorButtonMouseEnter:function(Y){var X=a(Y.currentTarget),Z=X.offset(),aa=this._getToolTipElement();
aa.style.visibility="hidden";
this._showElement(aa);
aa.innerHTML=X.find(K+g)[0].innerHTML;
aa.style.left=Z.left-a(aa).outerWidth(true)+L+"px";
aa.style.top=(Z.top-a(aa).outerHeight(true)-W)+"px";
aa.style.visibility="visible";
},_onErrorButtonMouseLeave:function(X){this._hideElement(this._getToolTipElement());
},_onToggleButtonClick:function(X){this._togglePanelBody(X);
},_extractCommandFromClassName:function(Z,aa){var X=a(Z),Y="removeFailedUpload";
if(X.hasClass(i)){Y="cancelUpload";
}else{if(a(aa).hasClass(D)){Y="removeSuccceededUpload";
this._uploadedFilesCount--;
}else{this._failedFilesCount--;
}}return Y;
},_getHeaderElement:function(){if(!this._headerElement){this._headerElement=a(this._element).find(K+s).get(0);
}return this._innerWrapElement;
},_getToolTipElement:function(){if(!this._toolTip){this._toolTip=a(this._element).find(K+F).get(0);
}return this._toolTip;
},_getValidFilesListElement:function(){var X=this._validFilesListWrapper;
if(!this._validList){this._validList=this._createChildListElement(X);
}return this._validList;
},_getInvalidFilesListElement:function(){var X=this._invalidFilesListWrapper;
if(!this._invalidList){this._invalidList=this._createChildListElement(X);
}return this._invalidList;
},_getProgressIcon:function(){if(!this._progressIcon){this._progressIcon=a(this._getHeaderElement()).find(K+z);
}return this._progressIcon;
},_getFileSizeInKB:function(X){return Math.round((X/1024)*100)/100;
},_getCurrentUploadingIndex:function(){var X=this._uploadedFilesCount+this._failedFilesCount+1;
return X;
},_toggleProgressIconVisibility:function(Y){var X=this._getProgressIcon();
X.css("display",Y?"inline-block":"none");
},_toggleActionButtonState:function(Y,Z){var X=Y.find(K+f);
X.removeClass(i);
X.addClass(B);
X.find(K+g).html(Z);
},_togglePanelBody:function(aa){var Z=a(aa.currentTarget),Y=Z.find(K+j),X=a(this._element).find(K+e);
if(Y.length>0){Y.removeClass(j);
Y.addClass(m);
if(X.length>0){this._hideElement(X[0]);
}}else{Y=Z.find(K+m);
Y.removeClass(m);
Y.addClass(j);
if(X.length>0){this._showElement(X[0]);
}}},_invalidateFileRow:function(X){this._getInvalidFilesListElement().appendChild(X);
this._createFakeButton(X);
},_updateUploadingLabel:function(){var X=a(this._element),Y=X.find(K+z)[0],Z=X.find(K+J)[0],ab=X.find(K+H)[0],aa=X.find(K+G)[0];
if(this._uploadedFilesCount+this._failedFilesCount!=this._totalFilesCount){if(Z.innerHTML!=this._localization.UploadingFilesMessage){Z.innerHTML=this._localization.UploadingFilesMessage+R;
this._showElement(Y,true);
}ab.innerHTML=this._getCurrentUploadingIndex();
}else{Z.innerHTML=this._localization.UploadedFilesMessage+R;
this._hideElement(Y);
ab.innerHTML=this._uploadedFilesCount;
}aa.innerHTML=this._totalFilesCount;
},_renderRow:function(){var X=document.createElement(O);
X.className=v+R+x;
this._renderStatusIcon(X);
this._renderFileName(X);
this._renderFileSize(X);
this._renderButtons(X);
return X;
},_renderSpan:function(X,Y){var Z=document.createElement(S);
Z.className=X;
if(Y){Z.innerHTML=Y;
}return Z;
},_renderInput:function(X,Z){var Y=document.createElement(M);
Y.className=X;
Y.setAttribute(T,Z);
return Y;
},_renderLabel:function(){var X=document.createElement(N);
return X;
},_renderButton:function(Y){var X=document.createElement(c);
X.className=Y;
X.setAttribute(T,c);
return X;
},_renderIcon:function(X){var Y=this._owner._cssClasses._rcuIcon;
if(X){Y+=R+X;
}return this._renderSpan(Y);
},_renderStatusIcon:function(X){X.appendChild(this._renderIcon(C));
},_renderFileName:function(X){var Y=this._fileInfo.name,Z=this._renderSpan(p,Y);
a(Z).attr("title",Y);
X.appendChild(Z);
},_renderFileSize:function(X){this._owner.get_view()._renderFileSize(X);
},_renderButtons:function(aa){var Z=this._renderSpan(h),X,Y=f+R+i,ab=this._localization;
aa.appendChild(Z);
X=this._renderButton(Y);
this._renderButtonContent(X,ab.Cancel);
Z.appendChild(X);
},_renderButtonContent:function(Y,X,Z){if(!Z){Y.appendChild(this._renderIcon());
}Y.appendChild(this._renderSpan(g,X));
},_renderProgressBar:function(ab,Y){var X="0%",aa=this._renderSpan(y,X);
aa.style.width=X;
var Z=this._renderSpan(A);
Z.appendChild(aa);
if(Y){a(ab).find(K+q).after(Z);
}else{a(ab).find(K+p).after(Z);
}},_createChildListElement:function(X){var Y=document.createElement(U);
Y.className=w;
X.appendChild(Y);
return Y;
},_createFakeButton:function(Z){var ab=!a(this._element).hasClass(t),Y=(ab)?l:o,aa=this._renderSpan(Y),X;
if(!ab){this._renderButtonContent(aa,Z._message,ab);
a(Z).find(K+h).prepend(aa);
}else{aa.innerHTML=Z._message;
X=a(Z).find(K+q);
if(X.length>0){X.after(aa);
}else{a(Z).find(K+p).after(aa);
}}}};
})($telerik.$);
