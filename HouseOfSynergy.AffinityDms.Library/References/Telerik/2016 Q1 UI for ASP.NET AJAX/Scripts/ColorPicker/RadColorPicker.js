...
if(!H){K+=L;
}var w=K+this._getMinimumPaletteWidth();
var m,n,I;
...
var o=$telerik.isTouchDevice?{left:u.x,top:u.y}:$telerik.getDocumentRelativeCursorPosition(n);
...
},_updateHexPreview:function(p,m,l,n){var o=this._rgbValuesToHex(p,m,l);
...
},set_selectedColor:function(x,v,u){if(!x||(x.toString().toLowerCase()=="null")){x=null;
}else{if(!this._validateHexValue(x)){return;
}if(x.indexOf("#")!=-1){x=x.substring(1,x.length);
}if(x.length==3){var o=function(y){return(y+y);
};
x=x.replace(/([0-9a-f])/gi,o);
}x="#"+x;
}var t=this._selectedColor;
if(!t){t=null;
}if(x!=null){x=x.toUpperCase();
}if(x!=t){var l=a.raiseCancellableControlEvent(this,"colorChanging",{oldColor:t,newColor:x});
if(l){var s=this._getBoxElementByColor(t);
if(s!=null){s.focus();
}var r=this._getBoxElementByColor(x);
if(r!=null){r.blur();
}return;
}}this._selectedColor=x;
this._previewCurrentColor(x);
if(this._showIcon){this._setIconColor(x);
}if(!v&&this._selectedContainer){if(this._isModeSelected(b.PaletteModes.WebPalette)){var p=this._emptyColorElement;
var n=this._getMarkedElement();
if(n!=null){if(n!=p){n.className="rcpColorBox";
}}n=this._getBoxElementByColor(x);
if(n!=null){this._setMarkedElement(n);
if(n!=p){n.className="rcpSelectedColor";
}}}else{var w=this._isModeSelected(b.PaletteModes.RGBSliders)?"_updateRgbSlidersUI":(this._isModeSelected(b.PaletteModes.HSB)?"_updateHslPaletteElementsUI":"_updateHsvPaletteElementsUI");
var m=(x===null||x===j)&&this._isModeSelected(b.PaletteModes.HSB)?{r:128,g:128,b:128}:this._getRgbFromHex(x);
this._millionColorsSliderAvoidRecursion=true;
this[w](m);
this._millionColorsSliderAvoidRecursion=false;
}}if(!u){this._updateRecentColors(x);
}this.updateClientState();
if(this._selectedColor!=t){var q=new b.ColorPickerColorChangeEventArgs(t);
...
}this._iconElement.style.backgroundColor=(l)?l:"#FFFFFF";
