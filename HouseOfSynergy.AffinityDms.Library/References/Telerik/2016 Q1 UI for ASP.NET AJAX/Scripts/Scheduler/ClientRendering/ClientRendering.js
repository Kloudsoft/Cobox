...
this._isClassic=this._owner.get_renderMode()===d.RenderMode.Classic;
this._isMobile=this._owner.get_renderMode()===d.RenderMode.Mobile;
...
if(!t.get_readOnlyResolved()){if(s){r.append("</div><a href='#' class='rsAptDelete' style='visibility: hidden;' title='delete'><span class='rsIcon rsIconDelete'></span></a>").append(u?"<span class='rsAptResize rsAptResizeStart'></span><span class='rsAptResize rsAptResizeEnd'></span>":"");
...
o.find("div.rsToolbar .rsViewToggle .rsText").text(t);
return;
...
if(q!=d.RecurrenceState.NotRecurring){if(q==d.RecurrenceState.Exception){return p?'<div class="rsAptRecurrenceException"></div>':'<span class="rsAptRecurrenceException"><span class="t-fi t-i-recurrence-exception"></span></span>';
}return p?'<div class="rsAptRecurrence"></div>':'<span class="rsAptRecurrence"><span class="t-fi t-i-recurrence"></span></span>';
}return"";
},_getReminderIconHtml:function(o){if(o.get_reminders&&o.get_reminders().get_count()>0){return this._isClassic?'<div class="rsAptReminder"></div>':'<span class="rsAptReminder"><span class="t-fi t-i-reminder"></span></span>';
}return"";
...
}if(t){this._mapAppointmentStyles(o,w.rsAptContent,{"border-width":"_borderWidth","border-style":"_borderStyle"});
}return w;
...
},_getHeaderDateText:function(){var r=this._model;
var q=this._getHeaderDateFormat();
var u=r.get_visibleRangeStart();
var s=r.get_visibleRangeEnd();
var t=r._getSlotDuration();
var v=this._getFormattedDateText(u,q);
var o;
var p;
if(b.areEqual(b.getDate(s),s)&&((t/l)%1)===0){o=b.add(s,-l);
}else{o=b.add(s,-t);
}p=this._getFormattedDateText(o,q);
return String.format("{0} - {1}",v,p);
...
if(!q.get_readOnlyResolved()){if(this._isLightweight){p.append("</div><a href='#' class='rsAptDelete' style='visibility: hidden;' title='delete'><span class='rsIcon rsIconDelete'></span></a>");
