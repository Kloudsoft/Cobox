...
if(!($telerik.isIE7||$telerik.isIE8)){var n=getComputedStyle(s[q]),m=0;
if(n){m=t=="offsetWidth"?(parseFloat(n.width)+parseFloat(n.paddingLeft)):parseFloat(n.height);
}u=m;
}else{u=s[q][t];
}o.size+=u;
Array.add(o,s[q]);
...
if(q.get_multiPage()){a(q.get_multiPage().get_element()).find(".rmpView").children("iframe").one("mouseenter",function(s){q._pageViewMouseEnter(s);
});
}if(q._scroller){q._attachScrollingHandlersWhenDrag();
}},_pageViewMouseEnter:function(m){var o=this;
var n;
if(o._dragInAction&&o._draggedTab){n=o._draggedTab.get_parent()._childListElement;
o._dragInAction=false;
o._detachSupplementaryDragHandlers(n);
o._disposeGhost();
o._disposeDropClues();
