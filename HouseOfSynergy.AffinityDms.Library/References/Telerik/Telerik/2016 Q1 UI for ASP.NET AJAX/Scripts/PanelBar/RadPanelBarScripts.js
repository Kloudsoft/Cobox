...
var e=a(this.get_expandHandleElement()).find(".t-fi");
...
e.removeClass("t-i-expand-down").addClass("t-i-expand-up");
}else{e.removeClass("t-i-expand-up").addClass("t-i-expand-down");
...
break;
case Sys.UI.Key.enter:this._onKeyboardEnter(f);
return true;
default:return true;
}f.preventDefault();
return false;
...
this.blur();
}}},_onKeyboardEnter:function(f){if(f.target&&f.target.tagName.toLowerCase()==="span"){f.target.click();
...
}var n=h?(h.target||h.srcElement):null;
if(n&&(a(n).hasClass("rpExpandHandle")||a(n.parentNode).hasClass("rpExpandHandle"))){return false;
