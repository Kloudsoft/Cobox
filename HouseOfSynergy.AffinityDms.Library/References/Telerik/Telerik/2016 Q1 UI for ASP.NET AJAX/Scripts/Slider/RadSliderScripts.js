...
}}var k=i?"rslDecrease":"rslIncrease";
...
var u=this._getSizeProperty();
var x=this._getUlSize();
var j=this._getAsymmetricAddOn(x[o?"width":"height"]);
var k={lastItemSize:0,asymmetricAddOn:j};
var r=0;
for(var n=0;
n<s;
n++){var p=q.getItem(n).get_element();
r=this._getItemSize(n,x,j);
$telerik.setSize(p,r);
if(n==s-1){k.lastItemSize=r[u];
}else{if(l&&(n>0)){var w=p.getElementsByTagName("SPAN");
var v=Math.max(r[u],12)+"px";
for(var m=0,t=w.length;
m<t;
m++){if(!o){w[m].style.lineHeight=v;
}else{w[m].style.lineHeight="";
