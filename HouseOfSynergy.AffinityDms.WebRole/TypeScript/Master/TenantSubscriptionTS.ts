module AffinityDms.Entities {
    ...
    //}
    export class TenantSubscriptionsTS {
        public GetSubscriptionById(event: any): any {
            var H_TenantId = <HTMLInputElement>document.getElementById("H_tenantId");
            var dd_subs = <HTMLSelectElement>document.getElementById("dd_subscription");
            ...
            $.ajax({
                ...
                success: function (response: any) {
                    if (response instanceof Object) {
                        var TenantSubscriptionsViewModel: any = response;
                        var tenantsubscriptionsarray = <Array<AffinityDms.Entities.TenantSubscription>>TenantSubscriptionsViewModel.TenantSubscriptions;
                        var subscriptionsarray: Array<AffinityDms.Entities.Subscription> = TenantSubscriptionsViewModel.Subscriptions;
                        if (tenantsubscriptionsarray == null) {
                            tenantsubscriptionsarray = new Array<AffinityDms.Entities.TenantSubscription>();
                        }
                        if (tenantsubscriptionsarray == null) {
                            subscriptionsarray = new Array<AffinityDms.Entities.Subscription>();
                        }
                        ...
                        var NumberOfPagesAllowed = (<number>subscriptionsarray[0].NumberOfPagesAllowed) + (<number>tenantsubscriptionsarray[0].NumberOfPagesAllowed);
                        var NumberOfUsersAllowed = (<number>subscriptionsarray[0].NumberOfUsersAllowed) + (<number>tenantsubscriptionsarray[0].NumberOfUsersAllowed);
                        var NumberOfTemplatesAllowed = (<number>subscriptionsarray[0].NumberOfTemplatesAllowed) + (<number>tenantsubscriptionsarray[0].NumberOfTemplatesAllowed);
                        var NumberOfFormsAllowed = (<number>subscriptionsarray[0].NumberOfFormsAllowed) + (<number>tenantsubscriptionsarray[0].NumberOfFormsAllowed);
                        var NumberOfPagesUsed = (<number>subscriptionsarray[0].NumberOfPagesUsed) + (<number>tenantsubscriptionsarray[0].NumberOfPagesUsed);
                        ...
                        var AllowScanning = (<boolean>subscriptionsarray[0].AllowScanning) || (<boolean>tenantsubscriptionsarray[0].AllowScanning);
                        var AllowBranding = (<boolean>subscriptionsarray[0].AllowBranding) || (<boolean>tenantsubscriptionsarray[0].AllowBranding);
        ...
        public ApplySubscription(event: any): any {
            var H_TenantId = <HTMLInputElement>document.getElementById("H_tenantId");
            var dd_subs = <HTMLSelectElement>document.getElementById("dd_subscription");
            var subsid = dd_subs.options[dd_subs.selectedIndex].getAttribute("value");

            var AllowScanning = <HTMLInputElement>document.getElementById("AllowScanning");
            var AllowBranding = <HTMLInputElement>document.getElementById("AllowBranding"); AllowBranding
            var AllowTemplateWorkflow = <HTMLInputElement>document.getElementById("AllowTemplateWorkflow");
            var StartDate = <HTMLInputElement>document.getElementById("StartDate");
            var ExpiryDate = <HTMLInputElement>document.getElementById("ExpiryDate");

            var divMessage = <HTMLInputElement>document.getElementById("divMessage");
            var divException = <HTMLInputElement>document.getElementById("divException");
        ...
        public ClearTenantSubscriptionSubmission(): any {
